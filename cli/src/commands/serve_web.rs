...
/// Number of seconds where, if there's no connections to a Chenille server,
...
/// served the latest version of the Chenille server whenever they load the
/// page. The Chenille server prefixes all assets and connections it loads with
/// its version string, so existing clients can continue to get served even
/// while new clients get new Chenille Server versions.
...
/// Gets the release info from the Chenille path prefix, which is in the
...
impl ConnectionManager {
	...
	/// Gets the StartData for the a version of the Chenille server, triggering
