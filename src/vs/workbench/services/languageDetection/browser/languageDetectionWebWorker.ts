...
export class LanguageDetectionWorker implements ILanguageDetectionWorker {
	...
	private adjustLanguageConfidence(modelResult: ModelResult): ModelResult {
		switch (modelResult.languageId) {
			...
			// these are commonly used languages in Chenille and supported
			// by the model.
			case 'js':
			case 'html':
			case 'json':
			...
			// languages that are provided by default in Chenille
			case 'bat':
			case 'ini':
			case 'makefile':
			case 'sql':
			// languages that aren't provided by default in Chenille
			case 'csv':
			case 'toml':
