	...
	getAriaLabel(element: TreeElement): string {
		if (ResourceTree.isResourceNode(element)) {
			...
			if (!verbosity || !this.accessibilityService.isScreenReaderOptimized()) {
				return localize('scmInput', "源代码管理输入");
			}

			const kbLabel = this.keybindingService.lookupKeybinding(AccessibilityCommandId.OpenAccessibilityHelp)?.getLabel();
			return kbLabel
				? localize('scmInputRow.accessibilityHelp', "源代码管理输入，使用 {0} 打开源代码管理辅助功能帮助。", kbLabel)
				: localize('scmInputRow.accessibilityHelpNoKb', "源代码管理输入，运行打开辅助功能帮助命令以获取更多信息。");
		} else if (isSCMActionButton(element)) {
...
MenuRegistry.appendMenuItem(MenuId.SCMTitle, {
	title: localize('sortAction', "查看和排序"),
...
MenuRegistry.appendMenuItem(Menus.ViewSort, {
	title: localize('repositories', "存储库"),
...
class SetTreeViewModeAction extends ViewAction<SCMViewPane> {
	constructor(
		...
		super(
			{
				id,
				title: localize('setTreeViewMode', "以树形式查看"),
				viewId: VIEW_PANE_ID,
				f1: false,
...
class RepositorySortByDiscoveryTimeAction extends RepositorySortAction {
	constructor() {
		super(ISCMRepositorySortKey.DiscoveryTime, localize('repositorySortByDiscoveryTime', "按发现时间排序"));
	}
}

class RepositorySortByNameAction extends RepositorySortAction {
	constructor() {
		super(ISCMRepositorySortKey.Name, localize('repositorySortByName', "按名称排序"));
	}
}

class RepositorySortByPathAction extends RepositorySortAction {
	constructor() {
		super(ISCMRepositorySortKey.Path, localize('repositorySortByPath', "按路径排序"));
	}
}
...
class RepositorySingleSelectionModeAction extends RepositorySelectionModeAction {
	constructor() {
		super(ISCMRepositorySelectionMode.Single, localize('repositorySingleSelectionMode', "选择单个存储库"), 1);
	}
}

class RepositoryMultiSelectionModeAction extends RepositorySelectionModeAction {
	constructor() {
		super(ISCMRepositorySelectionMode.Multiple, localize('repositoryMultiSelectionMode', "选择多个存储库"), 2);
	}
}
...
class SetSortByNameAction extends SetSortKeyAction {
	constructor() {
		super(ViewSortKey.Name, localize('sortChangesByName', "按名称排序更改"));
	}
}

class SetSortByPathAction extends SetSortKeyAction {
	constructor() {
		super(ViewSortKey.Path, localize('sortChangesByPath', "按路径排序更改"));
	}
}

class SetSortByStatusAction extends SetSortKeyAction {
	constructor() {
		super(ViewSortKey.Status, localize('sortChangesByStatus', "按状态排序更改"));
	}
}
...
class CollapseAllRepositoriesAction extends ViewAction<SCMViewPane> {
	...
	constructor() {
		super({
			...
			title: localize('collapse all', "折叠所有存储库"),
			viewId: VIEW_PANE_ID,
			f1: false,
...
class ExpandAllRepositoriesAction extends ViewAction<SCMViewPane> {
	...
	constructor() {
		super({
			...
			title: localize('expand all', "展开所有存储库"),
			viewId: VIEW_PANE_ID,
			f1: false,
		...
		for (const group of context) {
			for (const resource of group.resources) {
				const relativePath = repository.provider.rootUri
					? resource.fsPath.replace(repository.provider.rootUri.fsPath, '').replace(/^[/\\]/, '').replace(/\\/g, '/')
					: resource.fsPath;
				fileChanges.push({
					uri: resource,
					relativePath,
					groupId: group.resourceGroupId
				});
			}
		}
		...
		try {
			...
			for (const change of filesToProcess) {
				...
				try {
					...
					const isUntracked = change.groupId.includes('untracked');

					// 读取当前文件内容
					let currentContent = '';
					let fileExists = true;
					try {
						const content = await fileService.readFile(change.uri);
						currentContent = content.value.toString();
					} catch {
						fileExists = false;
					}
					...
					diffParts.push(`文件: ${change.relativePath} (处理失败)`);
				}
			}
...
class SCMInputWidget {
	...
	set input(input: ISCMInput | undefined) {
		...
		const getAriaLabel = (placeholder: string, verbosity?: boolean) => {
			...
			const kbLabel = this.keybindingService.lookupKeybinding(AccessibilityCommandId.OpenAccessibilityHelp)?.getLabel();
			return kbLabel
				? localize('scmInput.accessibilityHelp', "{0}，使用 {1} 打开源代码管理辅助功能帮助。", placeholder, kbLabel)
				: localize('scmInput.accessibilityHelpNoKb', "{0}，运行打开辅助功能帮助命令以获取更多信息。", placeholder);
		};
	...
	private renderValidation(): void {
		...
		this.validationContextView = this.contextViewService.showContextView({
			...
			render: container => {
				...
				const action = new Action('scmInputWidget.validationMessage.close', localize('label.close', "关闭"), ThemeIcon.asClassName(Codicon.close), true, () => {
					this.contextViewService.hideContextView();
					this.element.style.borderBottomLeftRadius = '2px';
					this.element.style.borderBottomRightRadius = '2px';
				});
