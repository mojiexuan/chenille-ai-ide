...
export class ChatInputPart extends Disposable implements IHistoryNavigationWidget {
		...
		if (!mode.isBuiltin) {
			...
			switch (this.currentModeKind) {
				case ChatModeKind.Agent:
					modeLabel = localize('chatInput.mode.agent', "(智能体)，编辑工作区中的文件。");
					break;
				case ChatModeKind.Edit:
					modeLabel = localize('chatInput.mode.edit', "(编辑)，编辑工作区中的文件。");
					break;
				case ChatModeKind.Ask:
				default:
					modeLabel = localize('chatInput.mode.ask', "(询问)，提问或输入 / 查看主题。");
					break;
			}
		}
		if (verbose) {
			return kbLabel
				? localize('actions.chat.accessibiltyHelp', "聊天输入 {0}{1} 按 Enter 发送请求。使用 {2} 获取聊天辅助功能帮助。", modeLabel, modelInfo, kbLabel)
				: localize('chatInput.accessibilityHelpNoKb', "聊天输入 {0}{1} 按 Enter 发送请求。使用聊天辅助功能帮助命令获取更多信息。", modeLabel, modelInfo);
		} else {
			return localize('chatInput.accessibilityHelp', "聊天输入 {0}{1}。", modeLabel, modelInfo);
		}
	}
	...
	render(container: HTMLElement, initialValue: string, widget: IChatWidget) {
		...
		this.addFilesToolbar.context = { widget, placeholder: localize('chatAttachFiles', '搜索要添加到请求中的文件和上下文') };
	...
	private renderChatEditingSessionWithEntries(
		...
		const button = this._chatEditsActionsDisposables.add(new ButtonWithIcon(overviewTitle, {
			supportIcons: true,
			secondary: true,
			ariaLabel: localize('chatEditingSession.toggleWorkingSet', '切换已更改的文件。'),
		}));
		...
		const topLevelStats = derived(reader => {
			...
			let baseLabel = entries.length === 1 ? localize('chatEditingSession.oneFile.1', '1 个文件已更改') : localize('chatEditingSession.manyFiles.1', '{0} 个文件已更改', entries.length);
			...
			if (added === 0 && removed === 0) {
				...
				baseLabel = sessionValue.length === 1 ? localize('chatEditingSession.oneFile.2', '1 个文件待合并') : localize('chatEditingSession.manyFiles.2', '{0} 个文件待合并', sessionValue.length);
		...
		store.add(autorun(reader => {
			...
			button.element.setAttribute('aria-label', localize('chatEditingSession.ariaLabelWithCounts', '{0}，添加了 {1} 行，删除了 {2} 行', baseLabel, added, removed));
		...
		store.add(autorun(reader => {
			...
			if (sessionFileEntries.length > 0) {
				if (editEntries.length > 0) {
					...
					allEntries.push({
						kind: 'divider',
						label: localize('chatEditingSession.allChanges', '工作树更改'),
	...
	async renderChatRelatedFiles() {
		...
		for (const { uri, description } of this.relatedFiles.value) {
			...
			const addButton = this._chatEditsActionsDisposables.add(new Button(anchor, {
				...
				secondary: true,
				hoverDelegate,
				ariaLabel: localize('chatEditingSession.addSuggestion', '添加建议 {0}', this.labelService.getUriLabel(uri, { relative: true })),
			}));
			addButton.icon = Codicon.add;
			addButton.setTitle(localize('chatEditingSession.addSuggested', '添加建议'));
