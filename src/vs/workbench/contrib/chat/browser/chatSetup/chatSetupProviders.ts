...
export class SetupAgent extends Disposable implements IChatAgentImplementation {
	...
	static registerBuiltInAgents(instantiationService: IInstantiationService, context: ChatEntitlementContext, controller: Lazy<ChatSetupController>): IDisposable {
		return instantiationService.invokeFunction(accessor => {
			...
			const { disposable: vscodeDisposable } = SetupAgent.doRegisterAgent(instantiationService, chatAgentService, 'setup.vscode', 'vscode', false, localize2('vscodeAgentDescription', "询问有关 Chenille 的问题").value, ChatAgentLocation.Chat, undefined, context, controller);
			disposables.add(vscodeDisposable);

			// Register workspace agent
			const { disposable: workspaceDisposable } = SetupAgent.doRegisterAgent(instantiationService, chatAgentService, 'setup.workspace', 'workspace', false, localize2('workspaceAgentDescription', "询问有关工作区的问题").value, ChatAgentLocation.Chat, undefined, context, controller);
			disposables.add(workspaceDisposable);

			// Register terminal agent
			const { disposable: terminalDisposable } = SetupAgent.doRegisterAgent(instantiationService, chatAgentService, 'setup.terminal.agent', 'terminal', false, localize2('terminalAgentDescription', "询问如何在终端中执行操作").value, ChatAgentLocation.Chat, undefined, context, controller);
			...
			disposables.add(SetupTool.registerTool(instantiationService, {
				...
				displayName: localize('setupToolDisplayName', "新建工作区"),
				modelDescription: 'Scaffold a new workspace in Chenille',
				userDescription: localize('setupToolsDescription', "在 Chenille 中搭建新工作区"),
	...
	private static readonly SETUP_NEEDED_MESSAGE = new MarkdownString(localize('settingUpCopilotNeeded', "你需要设置 GitHub Copilot 并登录才能使用聊天功能。"));
	private static readonly TRUST_NEEDED_MESSAGE = new MarkdownString(localize('trustNeeded', "你需要信任此工作区才能使用聊天功能。"));
	...
	private async doInvokeWithoutSetup(request: IChatAgentRequest, progress: (part: IChatProgress) => void, chatService: IChatService, languageModelsService: ILanguageModelsService, chatWidgetService: IChatWidgetService, chatAgentService: IChatAgentService, languageModelToolsService: ILanguageModelToolsService): Promise<IChatAgentResult> {
		...
		progress({
			...
			content: new MarkdownString(localize('waitingChat', "正在准备聊天...")),
		});
	...
	private async forwardRequestToChat(requestModel: IChatRequestModel, progress: (part: IChatProgress) => void, chatService: IChatService, languageModelsService: ILanguageModelsService, chatAgentService: IChatAgentService, chatWidgetService: IChatWidgetService, languageModelToolsService: ILanguageModelToolsService): Promise<void> {
		try {
			...
			progress({
				kind: 'warning',
				content: new MarkdownString(localize('copilotUnavailableWarning', "获取响应失败。请重试。"))
			});
		}
	}
	...
	private async doForwardRequestToChatWhenReady(requestModel: IChatRequestModel, progress: (part: IChatProgress) => void, chatService: IChatService, languageModelsService: ILanguageModelsService, chatAgentService: IChatAgentService, chatWidgetService: IChatWidgetService, languageModelToolsService: ILanguageModelToolsService): Promise<void> {
		...
		if (whenLanguageModelReady instanceof Promise || whenAgentReady instanceof Promise || whenToolsModelReady instanceof Promise) {
			const timeoutHandle = setTimeout(() => {
				progress({
					kind: 'progressMessage',
					content: new MarkdownString(localize('waitingChat2', "聊天即将就绪...")),
				});
			}, 10000);
			...
			try {
				...
				if (ready === 'timedout') {
					...
					if (this.chatEntitlementService.anonymous) {
						warningMessage = localize('chatTookLongWarningAnonymous', "聊天准备时间过长。请确保扩展 `{0}` 已安装并启用。", defaultChat.chatExtensionId);
					} else {
						warningMessage = localize('chatTookLongWarning', "聊天准备时间过长。请确保你已登录 {0}，并且扩展 `{1}` 已安装并启用。", defaultChat.provider.default.name, defaultChat.chatExtensionId);
					}
	...
	private async doInvokeWithSetup(request: IChatAgentRequest, progress: (part: IChatProgress) => void, chatService: IChatService, languageModelsService: ILanguageModelsService, chatWidgetService: IChatWidgetService, chatAgentService: IChatAgentService, languageModelToolsService: ILanguageModelToolsService): Promise<IChatAgentResult> {
		...
		const setupListener = Event.runAndSubscribe(this.controller.value.onDidChange, (() => {
			switch (this.controller.value.step) {
				case ChatSetupStep.SigningIn:
					progress({
						kind: 'progressMessage',
						content: new MarkdownString(localize('setupChatSignIn2', "正在登录 {0}...", ChatEntitlementRequests.providerId(this.configurationService) === defaultChat.provider.enterprise.id ? defaultChat.provider.enterprise.name : defaultChat.provider.default.name)),
					});
					break;
				case ChatSetupStep.Installing:
					progress({
						kind: 'progressMessage',
						content: new MarkdownString(localize('installingChat', "正在准备聊天...")),
					});
					break;
			}
		}));
		...
		if (typeof result?.success === 'boolean') {
			if (result.success) {
				...
				progress({
					kind: 'warning',
					content: new MarkdownString(localize('chatSetupError', "聊天设置失败。"))
				});
			}
		}
	...
	async provideCodeActions(model: ITextModel, range: Range | Selection): Promise<CodeActionList | undefined> {
		...
		if (range.isEmpty()) {
			...
			if (/^\s*$/.test(textAtLine)) {
				generateOrModifyTitle = localize('generate', "生成");
			...
			if (!/^\s*$/.test(textInSelection)) {
				generateOrModifyTitle = localize('modify', "修改");
