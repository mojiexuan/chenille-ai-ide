...
export class ChatSetupController extends Disposable {
	...
	async setup(options: IChatSetupControllerOptions = {}): Promise<ChatSetupResultValue> {
		...
		const title = localize('setupChatProgress', "正在准备聊天...");
	...
	private async signIn(options: IChatSetupControllerOptions): Promise<{ session: AuthenticationSession | undefined; entitlement: ChatEntitlement | undefined }> {
		...
		if (!session && !this.lifecycleService.willShutdown) {
			const { confirmed } = await this.dialogService.confirm({
				type: Severity.Error,
				message: localize('unknownSignInError', "登录 {0} 失败。是否重试？", ChatEntitlementRequests.providerId(this.configurationService) === defaultChat.provider.enterprise.id ? defaultChat.provider.enterprise.name : defaultChat.provider.default.name),
				detail: localize('unknownSignInErrorDetail', "您必须登录才能使用 AI 功能。"),
	...
	private async doInstallWithRetry(): Promise<void> {
		...
		if (error) {
			if (!this.lifecycleService.willShutdown) {
				const { confirmed } = await this.dialogService.confirm({
					type: Severity.Error,
					message: localize('unknownSetupError', "设置聊天时发生错误。是否重试？"),
	...
	private async handleEnterpriseInstance(): Promise<ChatSetupResultValue> {
		...
		const result = await this.quickInputService.input({
			prompt: localize('enterpriseInstance', "您的 {0} 实例是什么？", defaultChat.provider.enterprise.name),
			placeHolder: localize('enterpriseInstancePlaceholder', '例如 "octocat" 或 "https://octocat.ghe.com"...'),
			...
			validateInput: async value => {
				...
				if (domainRegEx.test(value)) {
					isSingleWord = true;
					return {
						content: localize('willResolveTo', "将解析为 {0}", `https://${value}.ghe.com`),
						severity: Severity.Info
					};
				} if (!fullUriRegEx.test(value)) {
					return {
						content: localize('invalidEnterpriseInstance', '您必须输入有效的 {0} 实例（例如 "octocat" 或 "https://octocat.ghe.com"）', defaultChat.provider.enterprise.name),
