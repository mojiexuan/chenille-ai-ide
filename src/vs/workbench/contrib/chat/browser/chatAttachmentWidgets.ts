...
abstract class AbstractChatAttachmentWidget extends Disposable {
	...
	protected attachClearButton() {
		...
		const clearButton = new Button(this.element, {
			...
			title: localize('chat.attachment.clearButton', "从上下文中移除")
		});
...
export class FileAttachmentWidget extends AbstractChatAttachmentWidget {
	...
	constructor(
		...
		let ariaLabel = range ? localize('chat.fileAttachmentWithRange', "已附加文件，{0}，第 {1} 行到第 {2} 行", friendlyName, range.startLineNumber, range.endLineNumber) : localize('chat.fileAttachment', "已附加文件，{0}", friendlyName);

		if (attachment.omittedState === OmittedState.Full) {
			ariaLabel = localize('chat.omittedFileAttachment', "已省略此文件：{0}", attachment.name);
	...
	private renderOmittedWarning(friendlyName: string, ariaLabel: string) {
		...
		hoverElement.textContent = localize('chat.fileAttachmentHover', "{0} 不支持此文件类型。", this.currentLanguageModel ? this.languageModelsService.lookupLanguageModel(this.currentLanguageModel.identifier)?.name : this.currentLanguageModel ?? '此模型');
...
export class TerminalCommandAttachmentWidget extends AbstractChatAttachmentWidget {
	...
	constructor(
		...
		const ariaLabel = localize('chat.terminalCommand', "终端命令，{0}", attachment.command);
...
function createTerminalCommandElements(
	...
	const commandTitle = dom.$('div', {}, typeof attachment.exitCode === 'number'
		? localize('chat.terminalCommandHoverCommandTitleExit', "命令：{0}，退出代码：{1}", attachment.command, attachment.exitCode)
		: localize('chat.terminalCommandHoverCommandTitle', "命令"));
	...
	if (attachment.output && attachment.output.trim().length > 0) {
		const outputTitle = dom.$('div', {}, localize('chat.terminalCommandHoverOutputTitle', "输出："));
	...
	const hint = dom.$('div', {}, localize('chat.terminalCommandHoverHint', "点击以在终端中聚焦此命令。"));
...
export class ImageAttachmentWidget extends AbstractChatAttachmentWidget {
	...
	constructor(
		...
		if (attachment.omittedState === OmittedState.Full) {
			ariaLabel = localize('chat.omittedImageAttachment', "已省略此图片：{0}", attachment.name);
		} else if (attachment.omittedState === OmittedState.Partial) {
			ariaLabel = localize('chat.partiallyOmittedImageAttachment', "已部分省略此图片：{0}", attachment.name);
		} else {
			ariaLabel = localize('chat.imageAttachment', "已附加图片，{0}", attachment.name);
		}
...
function createImageElements(resource: URI | undefined, name: string, fullName: string,
	...
	if ((!supportsVision && currentLanguageModel) || omittedState === OmittedState.Full) {
		...
		hoverElement.textContent = localize('chat.imageAttachmentHover', "{0} 不支持图片。", currentLanguageModelName ?? '此模型');
		disposable.add(hoverService.setupDelayedHover(element, {
			content: hoverElement,
			style: HoverStyle.Pointer,
		}));
		...
		if (resource) {
			const urlContainer = dom.$('a.chat-attached-context-url', {}, omittedState === OmittedState.Partial ? localize('chat.imageAttachmentWarning', "此 GIF 已被部分省略 - 将发送当前帧。") : fullName);
...
export class PasteAttachmentWidget extends AbstractChatAttachmentWidget {
	...
	constructor(
		...
		const ariaLabel = localize('chat.attachment', "已附加上下文，{0}", attachment.name);
		this.element.ariaLabel = ariaLabel;
...
export class DefaultChatAttachmentWidget extends AbstractChatAttachmentWidget {
	constructor(
		...
		this.element.ariaLabel = localize('chat.attachment', "已附加上下文，{0}", attachment.name);
...
export class PromptFileAttachmentWidget extends AbstractChatAttachmentWidget {
	...
	private updateLabel(attachment: IPromptFileVariableEntry) {
		...
		const ariaLabel = isPrompt
			? localize('chat.promptAttachment', "提示文件，{0}", friendlyName)
			: localize('chat.instructionsAttachment', "指令附件，{0}", friendlyName);
		const typeLabel = isPrompt
			? localize('prompt', "提示")
			: localize('instructions', "指令");
...
export class PromptTextAttachmentWidget extends AbstractChatAttachmentWidget {
	...
	constructor(
		...
		this.label.setLabel(localize('instructions.label', '附加指令'), undefined, undefined);
...
export class ToolSetOrToolItemAttachmentWidget extends AbstractChatAttachmentWidget {
	constructor(
		...
		this.element.style.cursor = 'pointer';
		this.element.ariaLabel = localize('chat.attachment', "已附加上下文，{0}", name);
...
export class NotebookCellOutputChatAttachmentWidget extends AbstractChatAttachmentWidget {
	...
	getAriaLabel(attachment: INotebookOutputVariableEntry): string {
		return localize('chat.NotebookImageAttachment', "已附加笔记本输出，{0}", attachment.name);
	}
	...
	private renderImageOutput(resource: URI, attachment: INotebookOutputVariableEntry) {
		let ariaLabel: string;
		if (attachment.omittedState === OmittedState.Full) {
			ariaLabel = localize('chat.omittedNotebookImageAttachment', "已省略此笔记本输出：{0}", attachment.name);
		} else if (attachment.omittedState === OmittedState.Partial) {
			ariaLabel = localize('chat.partiallyOmittedNotebookImageAttachment', "已部分省略此笔记本输出：{0}", attachment.name);
		} else {
...
export class ElementChatAttachmentWidget extends AbstractChatAttachmentWidget {
	constructor(
		...
		const ariaLabel = localize('chat.elementAttachment', "已附加元素，{0}", attachment.name);
		this.element.ariaLabel = ariaLabel;
		...
		const attachmentLabel = attachment.name;
		const withIcon = attachment.icon?.id ? `$(${attachment.icon.id})\u00A0${attachmentLabel}` : attachmentLabel;
		this.label.setLabel(withIcon, undefined, { title: localize('chat.clickToViewContents', "点击查看内容：{0}", attachmentLabel) });
...
export const chatAttachmentResourceContextKey = new RawContextKey<string>('chatAttachmentResource', undefined, { type: 'URI', description: localize('resource', "聊天附件资源的完整值，包括方案和路径") });
