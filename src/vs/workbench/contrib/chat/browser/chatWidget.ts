...
const DISCLAIMER = localize('chatDisclaimer', "AI 回复可能不准确。");
...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	*/
	...
	render(parent: HTMLElement): void {
		...
		scrollDownButton.setTitle(localize('scrollDownButtonLabel', "向下滚动"));
		...
		try {
			...
			if (!numItems) {
				...
				if (this.chatEntitlementService.anonymous && !this.chatEntitlementService.sentiment.installed) {
					...
					additionalMessage = new MarkdownString(localize({ key: 'settings', comment: ['{Locked="]({2})"}', '{Locked="]({3})"}'] }, "继续使用 {0} Copilot，即表示你同意 {1} 的[条款]({2})和[隐私声明]({3})。", providers.default.name, providers.default.name, product.defaultChatAgent.termsStatementUrl, product.defaultChatAgent.privacyStatementUrl), { isTrusted: true });
				} else {
	...
	private _getGenerateInstructionsMessage(): IMarkdownString {
		...
		if (!this._instructionFilesCheckPromise) {
			...
			// Use Chenille's idiomatic pattern for disposal-safe promise callbacks
	...
	private getWelcomeViewContent(additionalMessage: string | IMarkdownString | undefined): IChatViewWelcomeContent {
		if (this.isLockedToCodingAgent) {
			...
			const message = providerMessage
				...
				: (this._lockedAgent?.prefix === '@copilot '
					? new MarkdownString(localize('copilotCodingAgentMessage', "此聊天会话将转发给 {0} [编程代理]({1})，工作将在后台完成。", this._lockedAgent.prefix, 'https://aka.ms/coding-agent-docs') + DISCLAIMER, { isTrusted: true })
					: new MarkdownString(localize('genericCodingAgentMessage', "此聊天会话将转发给 {0} 编程代理，工作将在后台完成。", this._lockedAgent?.prefix) + DISCLAIMER));

			return {
				title: providerTitle ?? localize('codingAgentTitle', "委托给 {0}", this._lockedAgent?.prefix),
				message,
		...
		if (this.input.currentModeKind === ChatModeKind.Ask) {
			title = localize('chatDescription', "询问关于你的代码");
		...
		if (!this.chatEntitlementService.sentiment.installed) {
			...
			if (isEmpty) {
				return [
					{
						icon: Codicon.vscode,
						label: localize('chatWidget.suggestedPrompts.gettingStarted', "询问 @vscode"),
						prompt: localize('chatWidget.suggestedPrompts.gettingStartedPrompt', "@vscode 如何将主题更改为浅色模式？"),
					},
					{
						icon: Codicon.newFolder,
						label: localize('chatWidget.suggestedPrompts.newProject', "创建项目"),
						prompt: localize('chatWidget.suggestedPrompts.newProjectPrompt', "用 TypeScript 创建一个 #new Hello World 项目"),
					}
				];
			} else {
				return [
					{
						...
						label: localize('chatWidget.suggestedPrompts.buildWorkspace', "构建工作区"),
						prompt: localize('chatWidget.suggestedPrompts.buildWorkspacePrompt', "如何构建此工作区？"),
					},
					{
						icon: Codicon.gear,
						label: localize('chatWidget.suggestedPrompts.findConfig', "显示配置"),
						prompt: localize('chatWidget.suggestedPrompts.findConfigPrompt', "此项目的配置在哪里定义？"),
					}
				];
			}
		}
	...
	setModel(model: IChatModel | undefined): void {
		...
		if (this._lockedAgent) {
			...
			if (!placeholder) {
				placeholder = localize('chat.input.placeholder.lockedToAgent', "与 {0} 聊天", this._lockedAgent.id);
			}
