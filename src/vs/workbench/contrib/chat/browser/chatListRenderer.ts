...
export class ChatListItemRenderer extends Disposable implements ITreeRenderer<ChatTreeItem, FuzzyScore, IChatListItemTemplate> {
	...
	renderTemplate(container: HTMLElement): IChatListItemTemplate {
		...
		label.textContent = localize('checkpointRestore', '检查点已恢复');
	...
	private renderDetail(element: IChatResponseViewModel, templateData: IChatListItemTemplate): void {
		...
		if (element.agentOrSlashCommandDetected) {
			const msg = element.slashCommand ? localize('usedAgentSlashCommand', "使用了 {0} [[(不使用重新运行)]]", `${chatSubcommandLeader}${element.slashCommand.name}`) : localize('usedAgent', "[[(不使用重新运行)]]");
			...
			templateData.detail.textContent = localize('working', "正在处理");
		}
	}
	...
	private renderConfirmationAction(element: IChatRequestViewModel, templateData: IChatListItemTemplate) {
		...
		if (element.confirmation) {
			...
			dom.append(templateData.detail, $('span.confirmation-text', undefined, localize('chatConfirmationAction', '已选择"{0}"', element.confirmation)));
	...
	private renderChatContentPart(content: IChatRendererContent, templateData: IChatListItemTemplate, context: IChatContentPartRenderContext): IChatContentPart | undefined {
		try {
			...
			alert(`聊天错误：${toErrorMessage(err, false)}`);
			this.logService.error('ChatListItemRenderer#renderChatContentPart: error rendering content', toErrorMessage(err, true));
			const errorPart = this.instantiationService.createInstance(ChatErrorContentPart, ChatErrorLevel.Error, new MarkdownString(localize('renderFailMsg', "渲染内容失败") + `: ${toErrorMessage(err, false)}`), content, this.chatContentMarkdownRenderer);
			return {
	...
	private renderMarkdown(markdown: IChatMarkdownContent, templateData: IChatListItemTemplate, context: IChatContentPartRenderContext): IChatContentPart {
		...
		if (isRequestVM(element)) {
			...
			if (this.configService.getValue<string>('chat.editRequests') === 'inline' && this.rendererOptions.editable) {
				...
				this._register(this.hoverService.setupManagedHover(getDefaultHoverDelegate('element'), markdownPart.domNode, localize('requestMarkdownPartTitle', "点击编辑"), { trapFocus: true }));
			}
...
const voteDownDetailLabels: Record<ChatAgentVoteDownReason, string> = {
	[ChatAgentVoteDownReason.IncorrectCode]: localize('incorrectCode', "建议的代码不正确"),
	[ChatAgentVoteDownReason.DidNotFollowInstructions]: localize('didNotFollowInstructions', "没有遵循指示"),
	[ChatAgentVoteDownReason.MissingContext]: localize('missingContext', "缺少上下文"),
	[ChatAgentVoteDownReason.OffensiveOrUnsafe]: localize('offensiveOrUnsafe', "冒犯性或不安全"),
	[ChatAgentVoteDownReason.PoorlyWrittenOrFormatted]: localize('poorlyWrittenOrFormatted', "书写或格式不佳"),
	[ChatAgentVoteDownReason.RefusedAValidRequest]: localize('refusedAValidRequest', "拒绝了有效请求"),
	[ChatAgentVoteDownReason.IncompleteCode]: localize('incompleteCode', "代码不完整"),
	[ChatAgentVoteDownReason.WillReportIssue]: localize('reportIssue', "报告问题"),
	[ChatAgentVoteDownReason.Other]: localize('other', "其他"),
};
