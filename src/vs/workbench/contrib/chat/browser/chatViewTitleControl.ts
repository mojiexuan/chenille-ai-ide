...
export class ChatViewTitleControl extends Disposable {
	...
	private static readonly DEFAULT_TITLE = localize('chat', "聊天");
	...
	private getIconHoverContent(): string | undefined {
		...
		switch (sessionType) {
			case AgentSessionProviders.Background:
			case AgentSessionProviders.Cloud:
				return localize('backgroundSession', "{0} 代理会话", getAgentSessionProviderName(sessionType));
		}
	...
	private getTitleWithPrefix(): string {
		if (this.title) {
			return localize('chatTitleWithPrefixCustom', "聊天: {0}", this.title);
		}
