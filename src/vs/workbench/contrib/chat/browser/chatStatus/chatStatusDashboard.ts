...
}, localize('gaugeForeground', "仪表前景色。"));
...
registerColor('gauge.background', {
	...
	hcDark: Color.white,
	hcLight: Color.white
}, localize('gaugeBackground', "仪表背景色。"));
...
const gaugeWarningForeground = registerColor('gauge.warningForeground', {
	...
	hcDark: contrastBorder,
	hcLight: contrastBorder
}, localize('gaugeWarningForeground', "仪表警告前景色。"));
...
registerColor('gauge.warningBackground', {
	...
	hcDark: Color.white,
	hcLight: Color.white
}, localize('gaugeWarningBackground', "仪表警告背景色。"));
...
const gaugeErrorForeground = registerColor('gauge.errorForeground', {
	...
	hcDark: contrastBorder,
	hcLight: contrastBorder
}, localize('gaugeErrorForeground', "仪表错误前景色。"));
...
registerColor('gauge.errorBackground', {
	...
	hcDark: Color.white,
	hcLight: Color.white
}, localize('gaugeErrorBackground', "仪表错误背景色。"));
...
export class ChatStatusDashboard extends DomWidget {
	...
	private render(): void {
		...
		if (chatQuota || completionsQuota || premiumChatQuota) {
			addSeparator(localize('usageTitle', "Copilot 使用情况"), toAction({
				id: 'workbench.action.manageCopilot',
				label: localize('quotaLabel', "管理聊天"),
				tooltip: localize('quotaTooltip', "管理聊天"),
				class: ThemeIcon.asClassName(Codicon.settings),
				run: () => this.runCommandAndClose(() => this.openerService.open(URI.parse(defaultChat.manageSettingsUrl))),
			}));

			const completionsQuotaIndicator = completionsQuota && (completionsQuota.total > 0 || completionsQuota.unlimited) ? this.createQuotaIndicator(this.element, this._store, completionsQuota, localize('completionsLabel', "内联建议"), false) : undefined;
			const chatQuotaIndicator = chatQuota && (chatQuota.total > 0 || chatQuota.unlimited) ? this.createQuotaIndicator(this.element, this._store, chatQuota, localize('chatsLabel', "聊天消息"), false) : undefined;
			const premiumChatQuotaIndicator = premiumChatQuota && (premiumChatQuota.total > 0 || premiumChatQuota.unlimited) ? this.createQuotaIndicator(this.element, this._store, premiumChatQuota, localize('premiumChatsLabel', "高级请求"), true) : undefined;

			if (resetDate) {
				this.element.appendChild($('div.description', undefined, localize('limitQuota', "配额将于 {0} 重置。", resetDateHasTime ? this.dateTimeFormatter.value.format(new Date(resetDate)) : this.dateFormatter.value.format(new Date(resetDate)))));
			}
			...
			if (this.chatEntitlementService.entitlement === ChatEntitlement.Free && (Number(chatQuota?.percentRemaining) <= 25 || Number(completionsQuota?.percentRemaining) <= 25)) {
				...
				upgradeProButton.label = localize('upgradeToCopilotPro', "升级到 GitHub Copilot Pro");
		...
		// Anonymous Indicator
			...
			addSeparator(localize('anonymousTitle', "Copilot 使用情况"));

			this.createQuotaIndicator(this.element, this._store, localize('quotaLimited', "受限"), localize('completionsLabel', "内联建议"), false);
			this.createQuotaIndicator(this.element, this._store, localize('quotaLimited', "受限"), localize('chatsLabel', "聊天消息"), false);
		}
		...
		// Chat sessions
		{
			...
			if (inProgress.some(item => item.count > 0)) {
				...
				addSeparator(localize('chatAgentSessionsTitle', "智能体会话"), toAction({
					id: 'workbench.view.chat.status.sessions',
					label: localize('viewChatSessionsLabel', "查看智能体会话"),
					tooltip: localize('viewChatSessionsTooltip', "查看智能体会话"),
				...
				for (const { displayName, count } of inProgress) {
					if (count > 0) {
						const text = localize('inProgressChatSession', "$(loading~spin) {0} 进行中", displayName);
		...
		// Settings
		{
			...
			addSeparator(localize('inlineSuggestions', "内联建议"), chatSentiment.installed && !chatSentiment.disabled && !chatSentiment.untrusted ? toAction({
				id: 'workbench.action.openChatSettings',
				label: localize('settingsLabel', "设置"),
				tooltip: localize('settingsTooltip', "打开设置"),
		...
		// Model Selection
		{
			...
			if (provider) {
				...
				if (currentModel) {
					...
					modelContainer.appendChild($('span.model-text', undefined, localize('modelLabel', "模型: {0}", currentModel.name)));
					...
					toolbar.push([toAction({
						...
						label: localize('selectModel', "选择模型"),
						tooltip: localize('selectModel', "选择模型"),
		...
		if (this.canUseChat()) {
			...
			this.createCompletionsSnooze(snooze, localize('settings.snooze', "暂停"), this._store);
		}
		...
		// New to Chat / Signed out
		{
			...
			if (newUser || signedOut || disabled) {
				...
				if (newUser && anonymousUser) {
					descriptionText = new MarkdownString(localize({ key: 'activeDescriptionAnonymous', comment: ['{Locked="]({2})"}', '{Locked="]({3})"}'] }, "继续使用 {0} Copilot，即表示您同意 {1} 的[条款]({2})和[隐私声明]({3})", defaultChat.provider.default.name, defaultChat.provider.default.name, defaultChat.termsStatementUrl, defaultChat.privacyStatementUrl), { isTrusted: true });
					descriptionClass = `${descriptionClass}.terms`;
				} else if (newUser) {
					descriptionText = localize('activateDescription', "设置 Copilot 以使用 AI 功能。");
				} else if (anonymousUser) {
					descriptionText = localize('enableMoreDescription', "登录以启用更多 Copilot AI 功能。");
				} else if (disabled) {
					descriptionText = localize('enableDescription', "启用 Copilot 以使用 AI 功能。");
				} else {
					descriptionText = localize('signInDescription', "登录以使用 Copilot AI 功能。");
				}

				let buttonLabel: string;
				if (newUser) {
					buttonLabel = localize('enableAIFeatures', "使用 AI 功能");
				} else if (anonymousUser) {
					buttonLabel = localize('enableMoreAIFeatures', "启用更多 AI 功能");
				} else if (disabled) {
					buttonLabel = localize('enableCopilotButton', "启用 AI 功能");
				} else {
					buttonLabel = localize('signInToUseAIFeatures', "登录以使用 AI 功能");
				}
	...
	private renderContributedChatStatusItem(item: ChatStatusEntry): { element: HTMLElement; disposables: DisposableStore } {
		...
		this.renderHeader(itemElement, disposables, headerLabel, headerLink ? toAction({
			...
			tooltip: localize('learnMore', "了解更多"),
	...
	private createQuotaIndicator(container: HTMLElement, disposables: DisposableStore, quota: IQuotaSnapshot | string, label: string, supportsOverage: boolean): (quota: IQuotaSnapshot | string) => void {
		...
		if (supportsOverage && (this.chatEntitlementService.entitlement === ChatEntitlement.Pro || this.chatEntitlementService.entitlement === ChatEntitlement.ProPlus)) {
			...
			manageOverageButton.label = localize('enableAdditionalUsage', "管理付费高级请求");
		...
		const update = (quota: IQuotaSnapshot | string) => {
			...
			if (typeof quota === 'string') {
				...
				quotaValue.textContent = localize('quotaUnlimited', "已包含");
			} else if (quota.overageCount) {
				quotaValue.textContent = localize('quotaDisplayWithOverage', "+{0} 个请求", this.quotaOverageFormatter.value.format(quota.overageCount));
			} else {
			...
			if (supportsOverage) {
				if (typeof quota !== 'string' && quota?.overageEnabled) {
					overageLabel.textContent = localize('additionalUsageEnabled', "已启用额外付费高级请求。");
				} else {
					overageLabel.textContent = localize('additionalUsageDisabled', "已禁用额外付费高级请求。");
				}
			} else {
	...
	private createSettings(container: HTMLElement, disposables: DisposableStore): HTMLElement {
		...
		// --- Inline Suggestions
		{
			...
			this.createInlineSuggestionsSetting(globalSetting, localize('settings.codeCompletions.allFiles', "所有文件"), '*', disposables);
		...
		// --- Next edit suggestions
		{
			...
			this.createNextEditSuggestionsSetting(setting, localize('settings.nextEditSuggestions', "下一步编辑建议"), this.getCompletionsSettingAccessor(modeId), disposables);
		}
	...
	private createCompletionsSnooze(container: HTMLElement, label: string, disposables: DisposableStore): void {
		...
		const cancelAction = toAction({
			...
			label: localize('cancelSnooze', "取消暂停"),
		...
		const update = (isEnabled: boolean) => {
			...
			if (!isEnabled || timeLeftMs <= 0) {
				timerDisplay.textContent = localize('completions.snooze5minutesTitle', "隐藏建议 5 分钟");
				timerDisplay.title = '';
				button.label = label;
				button.setTitle(localize('completions.snooze5minutes', "隐藏内联建议 5 分钟"));
				return true;
			}
			...
			timerDisplay.textContent = `${minutes}:${seconds < 10 ? '0' : ''}${seconds} ${localize('completions.remainingTime', "剩余")}`;
			timerDisplay.title = localize('completions.snoozeTimeDescription', "内联建议在剩余时间内被隐藏");
			button.label = localize('completions.plus5min', "+5 分钟");
			button.setTitle(localize('completions.snoozeAdditional5minutes', "再暂停 5 分钟"));
	...
	private async showModelPicker(provider: languages.InlineCompletionsProvider): Promise<void> {
		...
		const items: IQuickPickItem[] = modelInfo.models.map(model => ({
			id: model.id,
			label: model.name,
			description: model.id === modelInfo.currentModelId ? localize('currentModel.description', "当前选中") : undefined,
			picked: model.id === modelInfo.currentModelId
		}));

		const selected = await this.quickInputService.pick(items, {
			placeHolder: localize('selectModelFor', "为 {0} 选择模型", provider.displayName || 'inline completions'),
			canPickMany: false
		});
