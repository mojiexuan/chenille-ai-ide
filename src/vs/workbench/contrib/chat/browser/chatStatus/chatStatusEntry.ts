...
export class ChatStatusBarEntry extends Disposable implements IWorkbenchContribution {
	...
	private getEntryProps(): IStatusbarEntry {
		let text = '$(copilot)';
		let ariaLabel = localize('chatStatusAria', "Copilot 状态");
		...
		if (isNewUser(this.chatEntitlementService)) {
			...
			if (
				...
				const finishSetup = localize('finishSetup', "完成设置");
			...
			if (this.chatEntitlementService.sentiment.disabled || this.chatEntitlementService.sentiment.untrusted) {
				...
				ariaLabel = localize('copilotDisabledStatus', "Copilot 已禁用");
			}
			...
			// Sessions in progress
				...
				if (this.runningSessionsCount > 1) {
					ariaLabel = localize('chatSessionsInProgressStatus', "{0} 个智能体会话正在进行中", this.runningSessionsCount);
				} else {
					ariaLabel = localize('chatSessionInProgressStatus', "1 个智能体会话正在进行中");
				}
			}

			// Signed out
			else if (this.chatEntitlementService.entitlement === ChatEntitlement.Unknown) {
				const signedOutWarning = localize('notSignedIn', "已退出登录");
			...
			// Free Quota Exceeded
				...
				if (chatQuotaExceeded && !completionsQuotaExceeded) {
					quotaWarning = localize('chatQuotaExceededStatus', "聊天配额已用完");
				} else if (completionsQuotaExceeded && !chatQuotaExceeded) {
					quotaWarning = localize('completionsQuotaExceededStatus', "内联建议配额已用完");
				} else {
					quotaWarning = localize('chatAndCompletionsQuotaExceededStatus', "配额已用完");
				}
			...
			// Completions Disabled
				...
				text = '$(copilot-unavailable)';
				ariaLabel = localize('completionsDisabledStatus', "内联建议已禁用");
			}

			// Completions Snoozed
			else if (this.completionsService.isSnoozing()) {
				text = '$(copilot-snooze)';
				ariaLabel = localize('completionsSnoozedStatus', "内联建议已暂停");
			}
		}

		const baseResult = {
			name: localize('chatStatus', "Copilot 状态"),
			text,
			ariaLabel,
