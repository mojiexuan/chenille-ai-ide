...
class SessionsAccessibilityProvider {
	getWidgetAriaLabel(): string {
		return nls.localize('chatSessions', '聊天会话');
	}
...
export class SessionsViewPane extends ViewPane {
	...
	private getChatSessionDropdown(defaultAction: IAction, options: IBaseActionViewItemOptions) {
		...
		const dropdownAction = toAction({
			...
			label: nls.localize('chatSession.selectOption', '更多...'),
			...
			await this.progressService.withProgress(
				{
					...
					title: nls.localize('chatSessions.refreshing', '正在刷新聊天会话...'),
				},
				async () => {
	...
	private async loadDataWithProgress(): Promise<void> {
		...
		try {
			await this.progressService.withProgress(
				{
					location: this.id, // Use the view ID as the progress location
					title: nls.localize('chatSessions.loading', '正在加载聊天会话...'),
				},
				async () => {
			...
			dataSource,
			{
				dnd: {
					...
					getDragLabel: (elements: ChatSessionItemWithProvider[]) => {
						...
						return nls.localize('chatSessions.dragLabel', "{0} 个智能体会话", elements.length);
					},
					drop: () => { },
					onDragOver: () => false,
					dispose: () => { },
		...
		const items: IGettingStartedItem[] = [
			{
				...
				label: nls.localize('chatSessions.installExtensions', "安装智能体..."),
				icon: Codicon.extensions,
				commandId: 'chat.sessions.gettingStarted'
			},
			{
				id: 'learn-more',
				label: nls.localize('chatSessions.learnMoreGHCodingAgent', "了解更多关于 GitHub Copilot 编码智能体"),
