...
const chatOutputRendererContributionSchema = {
	...
	properties: {
		viewType: {
			type: 'string',
			description: nls.localize('chatOutputRenderer.viewType', '渲染器的唯一标识符。'),
		},
		mimeTypes: {
			type: 'array',
			description: nls.localize('chatOutputRenderer.mimeTypes', '此渲染器可以处理的 MIME 类型'),
			items: {
				type: 'string'
			}
		}
...
const chatOutputRenderContributionPoint = ExtensionsRegistry.registerExtensionPoint<IChatOutputRendererContribution[]>({
	...
	jsonSchema: {
		description: nls.localize('vscode.extension.contributes.chatOutputRenderer', '为聊天输出中的特定 MIME 类型贡献渲染器'),
		type: 'array',
