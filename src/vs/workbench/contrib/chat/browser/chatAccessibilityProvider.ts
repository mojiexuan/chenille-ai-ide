...
export const getToolConfirmationAlert = (accessor: ServicesAccessor, toolInvocation: IChatToolInvocation[]) => {
	...
	const text = toolInvocation.map(v => {
		...
		if (state.type === IChatToolInvocation.StateKind.WaitingForPostApproval) {
			return {
				title: localize('toolPostApprovalTitle', "批准工具结果"),
	...
	let message = acceptKb && cancelKb
		? localize('toolInvocationsHintKb', "需要聊天确认：{0}。按 {1} 接受或按 {2} 取消。", text.map(t => t.title).join(', '), acceptKb, cancelKb)
		: localize('toolInvocationsHint', "需要聊天确认：{0}", text.map(t => t.title).join(', '));

	if (text.some(t => t.detail)) {
		message += ' ' + localize('toolInvocationsHintDetails', "详情：{0}", text.map(t => t.detail ? t.detail : '').join(' '));
	}
