...
const RUN_WITHOUT_APPROVAL = localize('runWithoutApproval', "无需批准");
const CONTINUE_WITHOUT_REVIEWING_RESULTS = localize('continueWithoutReviewingResults', "无需审查结果");
...
export class LanguageModelToolsConfirmationService extends Disposable implements ILanguageModelToolsConfirmationService {
	...
	getPreConfirmActions(ref: ILanguageModelToolConfirmationRef): ILanguageModelToolConfirmationActions[] {
		...
		actions.push(
			{
				label: localize('allowSession', '在此会话中允许'),
				detail: localize('allowSessionTooltip', '允许此工具在此会话中运行而无需确认。'),
			...
			{
				label: localize('allowWorkspace', '在此工作区允许'),
				detail: localize('allowWorkspaceTooltip', '允许此工具在此工作区中运行，无需确认。'),
				select: async () => {
			...
			{
				label: localize('allowGlobally', '总是允许'),
				detail: localize('allowGloballyTooltip', '始终允许此工具在未经确认的情况下运行。'),
				select: async () => {
		...
		if (ref.source.type === 'mcp') {
			...
			actions.push(
				{
					label: localize('allowServerSession', '在此会话中允许来自 {0} 的工具', serverLabel),
					detail: localize('allowServerSessionTooltip', '允许此服务器的所有工具在此会话中运行而无需确认。'),
					divider: true,
					select: async () => {
				...
				{
					label: localize('allowServerWorkspace', '在此工作区中允许来自 {0} 的工具', serverLabel),
					detail: localize('allowServerWorkspaceTooltip', '允许此服务器的所有工具在此工作区中运行而无需确认。'),
					select: async () => {
				...
				{
					label: localize('allowServerGlobally', '始终允许来自 {0} 的工具', serverLabel),
					detail: localize('allowServerGloballyTooltip', '始终允许此服务器的所有工具运行而无需确认。'),
					select: async () => {
	...
	getPostConfirmActions(ref: ILanguageModelToolConfirmationRef): ILanguageModelToolConfirmationActions[] {
		...
		actions.push(
			{
				label: localize('allowSessionPost', '在此会话中允许无需审查'),
				detail: localize('allowSessionPostTooltip', '允许此工具的结果在此会话中无需确认即可发送。'),
				divider: !!actions.length,
				select: async () => {
			...
			{
				label: localize('allowWorkspacePost', '在此工作区中允许无需审查'),
				detail: localize('allowWorkspacePostTooltip', '允许此工具的结果在此工作区中无需确认即可发送。'),
				select: async () => {
			...
			{
				label: localize('allowGloballyPost', '始终允许无需审查'),
				detail: localize('allowGloballyPostTooltip', '始终允许此工具的结果无需确认即可发送。'),
				select: async () => {
		...
		if (ref.source.type === 'mcp') {
			...
			actions.push(
				{
					label: localize('allowServerSessionPost', '在此会话中允许来自 {0} 的工具无需审查', serverLabel),
					detail: localize('allowServerSessionPostTooltip', '允许此服务器所有工具的结果在此会话中无需确认即可发送。'),
					divider: true,
					select: async () => {
				...
				{
					label: localize('allowServerWorkspacePost', '在此工作区中允许来自 {0} 的工具无需审查', serverLabel),
					detail: localize('allowServerWorkspacePostTooltip', '允许此服务器所有工具的结果在此工作区中无需确认即可发送。'),
					select: async () => {
				...
				{
					label: localize('allowServerGloballyPost', '始终允许来自 {0} 的工具无需审查', serverLabel),
					detail: localize('allowServerGloballyPostTooltip', '始终允许此服务器所有工具的结果无需确认即可发送。'),
					select: async () => {
	...
	manageConfirmationPreferences(tools: readonly IToolData[], options?: { defaultScope?: 'workspace' | 'profile' | 'session' }): void {
			...
			for (const [serverId, serverInfo] of serversWithTools) {
				...
				if (hasAnyPost) {
					serverChildren.unshift({
						...
						label: localize('continueWithoutReviewing', "继续而不审查任何工具结果"),
				...
				if (hasAnyPre) {
					serverChildren.unshift({
						...
						serverId,
						iconClass: ThemeIcon.asClassName(Codicon.play),
						label: localize('runToolsWithoutApproval', "运行任何工具而无需批准"),
		...
		if (currentScope !== 'session') {
			const scopeToggle = disposables.add(new Toggle({
				title: localize('workspaceScope', "仅为此工作区配置"),
				icon: Codicon.folder,
		...
		const updatePlaceholder = () => {
			if (currentScope === 'session') {
				quickTree.placeholder = localize('configureSessionToolApprovals', "配置会话工具批准");
			} else {
				quickTree.placeholder = currentScope === 'workspace'
					? localize('configureWorkspaceToolApprovals', "配置工作区工具批准")
					: localize('configureGlobalToolApprovals', "配置全局工具批准");
			}
		};
		updatePlaceholder();
