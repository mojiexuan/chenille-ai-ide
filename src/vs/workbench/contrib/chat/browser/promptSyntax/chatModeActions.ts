...
abstract class ConfigAgentActionImpl extends Action2 {
	public override async run(accessor: ServicesAccessor): Promise<void> {
		...
		const placeholder = localize('configure.agent.prompts.placeholder', "选择要打开的自定义智能体并配置在智能体选择器中的可见性");
...
function createPickerConfigureAgentsActionConfig(disabled: boolean) {
	const config = {
		...
		title: localize2('select-agent', "配置自定义智能体..."),
		tooltip: disabled ? localize('managedByOrganization', "由您的组织管理") : undefined,
...
function createManageAgentsActionConfig(disabled: boolean) {
	const base = {
		...
		title: localize2('configure-agents', "配置自定义智能体..."),
		shortTitle: localize('configure-agents.short', "自定义智能体"),
	...
	return disabled ? { ...base, tooltip: localize('managedByOrganization', "由您的组织管理") } : base;
}
