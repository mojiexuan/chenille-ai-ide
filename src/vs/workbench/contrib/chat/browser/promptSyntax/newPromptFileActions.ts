	...
	public override async run(accessor: ServicesAccessor) {
		...
		notificationService.prompt(
			...
			localize(
				...
				"是否要使用设置同步来备份和同步您的用户提示、指令和自定义智能体文件？",
			),
			[
				{
					label: localize('enable.capitalized', "启用"),
					run: () => {
				...
				{
					label: localize('learnMore.capitalized', "了解更多"),
					run: () => {
...
class NewPromptFileAction extends AbstractNewPromptFileAction {
	constructor() {
		super(NEW_PROMPT_COMMAND_ID, localize('commands.new.prompt.local.title', "新建提示文件..."), PromptsType.prompt);
	}
}

class NewInstructionsFileAction extends AbstractNewPromptFileAction {
	constructor() {
		super(NEW_INSTRUCTIONS_COMMAND_ID, localize('commands.new.instructions.local.title', "新建指令文件..."), PromptsType.instructions);
	}
}

class NewAgentFileAction extends AbstractNewPromptFileAction {
	constructor() {
		super(NEW_AGENT_COMMAND_ID, localize('commands.new.agent.local.title', "新建自定义智能体..."), PromptsType.agent);
	}
}
...
class NewUntitledPromptFileAction extends Action2 {
	constructor() {
		super({
			...
			title: localize2('commands.new.untitled.prompt.title', "新建无标题提示文件"),
			f1: true,
			precondition: ChatContextKeys.enabled,
			category: CHAT_CATEGORY,
			keybinding: {
