		...
		if (this.data.kind === 'symbol') {
			...
			if (location.range) {
				if (location.range.startLineNumber === location.range.endLineNumber) {
					element.setAttribute('aria-label', nls.localize('chat.inlineAnchor.ariaLabel.line', "{0} 第 {1} 行", filePathLabel, location.range.startLineNumber));
				} else {
					element.setAttribute('aria-label', nls.localize('chat.inlineAnchor.ariaLabel.range', "{0} 第 {1} 行到第 {2} 行", filePathLabel, location.range.startLineNumber, location.range.endLineNumber));
				}
			}
		}
...
registerAction2(class AddFileToChatAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: nls.localize2('actions.attach.label', "添加文件到聊天"),
			menu: [{
...
registerAction2(class CopyResourceAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: nls.localize2('actions.copy.label', "复制"),
			f1: false,
...
registerAction2(class OpenToSideResourceAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: nls.localize2('actions.openToSide.label', "在侧边打开"),
			f1: false,
			precondition: chatAttachmentResourceContextKey,
			keybinding: {
...
registerAction2(class GoToTypeDefinitionsAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: {
				...nls.localize2('goToTypeDefinitions.label', "转到类型定义"),
...
registerAction2(class GoToImplementations extends Action2 {
	...
	constructor() {
		super({
			...
			title: {
				...nls.localize2('goToImplementations.label', "转到实现"),
				mnemonicTitle: nls.localize({ key: 'miGotoImplementations', comment: ['&& denotes a mnemonic'] }, "转到实现(&&I)"),
			},
			menu: [MenuId.ChatInlineSymbolAnchorContext, MenuId.ChatInputSymbolAttachmentContext].map(id => ({
				id,
				group: '4_symbol_nav',
