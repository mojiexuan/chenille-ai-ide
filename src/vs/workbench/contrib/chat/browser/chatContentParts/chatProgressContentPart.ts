	...
	private createApprovalMessage(): IMarkdownString | undefined {
		...
		switch (reason.type) {
			case ToolConfirmKind.Setting:
				md = localize('chat.autoapprove.setting', '由 {0} 自动批准', createMarkdownCommandLink({ title: '`' + reason.id + '`', id: 'workbench.action.openSettings', arguments: [reason.id] }, false));
				break;
			case ToolConfirmKind.LmServicePerTool:
				md = reason.scope === 'session'
					? localize('chat.autoapprove.lmServicePerTool.session', '已为本会话自动批准')
					: reason.scope === 'workspace'
						? localize('chat.autoapprove.lmServicePerTool.workspace', '已为此工作区自动批准')
						: localize('chat.autoapprove.lmServicePerTool.profile', '已为此配置文件自动批准');
				md += ' (' + createMarkdownCommandLink({ title: localize('edit', '编辑'), id: 'workbench.action.chat.editToolApproval', arguments: [reason.scope] }) + ')';
				break;
...
export class ChatWorkingProgressContentPart extends ChatProgressContentPart implements IChatContentPart {
	constructor(
		...
		const progressMessage: IChatProgressMessage = {
			...
			content: new MarkdownString().appendText(localize('workingMessage', "正在处理..."))
		};
		...
		this._register(languageModelToolsService.onDidPrepareToolCallBecomeUnresponsive(e => {
			if (context.element.sessionId === e.sessionId) {
				this.updateMessage(new MarkdownString(localize('toolCallUnresponsive', "正在等待工具 '{0}' 响应...", e.toolData.displayName)));
			}
		}));
	}
