	...
	constructor(
		...
		switch (chatEntitlementService.entitlement) {
			...
			case ChatEntitlement.ProPlus:
				primaryButtonLabel = localize('enableAdditionalUsage', "管理付费高级请求");
				break;
			case ChatEntitlement.Free:
				primaryButtonLabel = localize('upgradeToCopilotPro', "升级到 GitHub Copilot Pro");
				break;
		}
		...
		const addWaitWarningIfNeeded = () => {
			...
			dom.append(messageContainer, $('.chat-quota-wait-warning', undefined, localize('waitWarning', "更改可能需要几分钟才能生效。")));
		};
		...
		const addRetryButtonIfNeeded = () => {
			...
			retryButton.label = localize('clickToContinue', "点击重试");
