...
class TodoListRenderer implements IListRenderer<IChatTodo, ITodoListTemplate> {
	...
	private getStatusText(status: string): string {
		switch (status) {
			case 'completed':
				return localize('chat.todoList.status.completed', '已完成');
			case 'in-progress':
				return localize('chat.todoList.status.inProgress', '进行中');
			case 'not-started':
			default:
				return localize('chat.todoList.status.notStarted', '未开始');
		}
	}
...
export class ChatTodoListWidget extends Disposable {
	...
	private createChatTodoWidget(): HTMLElement {
		...
		this.titleElement.textContent = localize('chat.todoList.title', '待办事项');
			...
			this._todoList = this._register(this.instantiationService.createInstance(
				...
				[new TodoListRenderer(this.configurationService)],
				{
					...
					accessibilityProvider: {
						...
						getWidgetAriaLabel: () => localize('chatTodoList', '聊天待办事项列表')
					}
				}
			));
		}
	...
	private updateClearButtonState(): void {
		...
		if (shouldDisable) {
			this.clearButton.setTitle(localize('chat.todoList.clearButton.disabled', '任务进行中时无法清除待办事项'));
		} else {
			this.clearButton.setTitle(localize('chat.todoList.clearButton', '清除所有待办事项'));
		}
	}
	...
	private updateTitleElement(titleElement: HTMLElement, todoList: IChatTodo[]): void {
		...
		const expandButtonLabel = this._isExpanded
			? localize('chat.todoList.collapseButton', '折叠待办事项')
			: localize('chat.todoList.expandButton', '展开待办事项');
		...
		if (this._isExpanded) {
			...
			titleText.textContent = totalCount > 0 ?
				localize('chat.todoList.titleWithCount', '待办事项 ({0}/{1})', currentTaskNumber, totalCount) :
				localize('chat.todoList.title', '待办事项');
			...
			// Show "Done" when all tasks are completed
				...
				doneText.textContent = localize('chat.todoList.titleWithCount', '待办事项 ({0}/{1})', totalCount, totalCount);
