	...
	render(uri: URI, fromSubagent?: boolean): void {
		...
		this.progressStore.add(autorun(r => {
			...
			if (isStreaming.read(r)) {
				...
				statusLabelEl.textContent = localize('chat.codeblock.applyingEdits', '正在应用编辑');
				...
				if (showAnimation) {
					...
					labelDetail.textContent = rwRatio === 0 || !rwRatio ? localize('chat.codeblock.generating', "正在生成编辑...") : localize('chat.codeblock.applyingPercentage', "({0}%)...", rwRatio);
				}
			} else {
				const statusCodeicon = Codicon.check;
				statusIconClasses = ThemeIcon.asClassNameArray(statusCodeicon);
				statusIconEl.classList.add(...statusIconClasses);
				statusLabelEl.textContent = localize('chat.codeblock.edited', '已编辑');
		...
		this.progressStore.add(autorunSelfDisposable(r => {
			...
			if (changes && !changes?.identical && !changes?.quitEarly) {
				...
				const insertionsFragment = changes.added === 1 ? localize('chat.codeblock.insertions.one', "1 处插入") : localize('chat.codeblock.insertions', "{0} 处插入", changes.added);
				const deletionsFragment = changes.removed === 1 ? localize('chat.codeblock.deletions.one', "1 处删除") : localize('chat.codeblock.deletions', "{0} 处删除", changes.removed);
				const summary = localize('summary', '已编辑 {0}，{1}，{2}', iconText, insertionsFragment, deletionsFragment);
