...
class ChatTerminalToolOutputSection extends Disposable {
	...
	public updateAriaLabel(): void {
		...
		const ariaLabel = localize('chatTerminalOutputAriaLabel', '{0} 的终端输出', commandText);
	...
	public getCommandAndOutputAsText(): string | undefined {
		...
		const commandHeader = localize('chatTerminalOutputAccessibleViewHeader', '命令：{0}', commandText);
		if (command) {
			const rawOutput = command.getOutput();
			if (!rawOutput || rawOutput.trim().length === 0) {
				return `${commandHeader}\n${localize('chat.terminalOutputEmpty', '该命令没有产生输出。')}`;
			}
		...
		if (!snapshot) {
			return `${commandHeader}\n${localize('chatTerminalOutputUnavailable', '命令输出已不可用。')}`;
		}
		...
		if (snapshot.truncated) {
			outputText += `\n${localize('chatTerminalOutputTruncated', '输出已截断。')}`;
		}
		return `${commandHeader}\n${outputText}`;
	}
	...
	private async _renderLiveOutput(liveTerminalInstance: ITerminalInstance, command: ITerminalCommand): Promise<boolean> {
		...
		if (!result || result.lineCount === 0) {
			this._showEmptyMessage(localize('chat.terminalOutputEmpty', '该命令没有产生输出。'));
		} else {
	...
	private _renderUnavailableMessage(liveTerminalInstance: ITerminalInstance | undefined): void {
		...
		if (!liveTerminalInstance) {
			this._showEmptyMessage(localize('chat.terminalOutputTerminalMissing', '终端已不可用。'));
		} else {
			this._showEmptyMessage(localize('chat.terminalOutputCommandMissing', '命令信息不可用。'));
		}
	}
...
export class ToggleChatTerminalOutputAction extends Action implements IAction {
	...
	constructor(
		...
		super(
			...
			localize('showTerminalOutput', '显示输出'),
	...
	private _updatePresentation(): void {
		if (this._expanded) {
			this.label = localize('hideTerminalOutput', '隐藏输出');
			this.class = ThemeIcon.asClassName(Codicon.chevronDown);
		} else {
			this.label = localize('showTerminalOutput', '显示输出');
...
export class FocusChatInstanceAction extends Action implements IAction {
	constructor(
		...
		super(
			...
			isTerminalHidden ? localize('showTerminal', '显示并聚焦终端') : localize('focusTerminal', '聚焦终端'),
	...
	public override async run() {
		this.label = localize('focusTerminal', '聚焦终端');
		this._updateTooltip();
