...
export class ExtensionsInstallConfirmationWidgetSubPart extends BaseChatToolInvocationSubPart {
	...
	constructor(
		...
		if (toolInvocation.state.get().type === IChatToolInvocation.StateKind.WaitingForConfirmation) {
			const allowLabel = localize('allow', "允许");
			...
			const cancelLabel = localize('cancel', "取消");
			...
			const confirmWidget = this._register(instantiationService.createInstance(
				...
				context,
				{
					title: toolInvocation.confirmationMessages?.title ?? localize('installExtensions', "安装扩展"),
					message: toolInvocation.confirmationMessages?.message ?? localize('installExtensionsConfirmation', "点击扩展上的安装按钮，完成后按允许。"),
					buttons,
				}
			));
