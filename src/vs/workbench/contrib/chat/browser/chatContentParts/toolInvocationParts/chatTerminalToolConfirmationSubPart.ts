...
export class ChatTerminalToolConfirmationSubPart extends BaseChatToolInvocationSubPart {
	...
	constructor(
		...
		if (autoApproveEnabled) {
			...
			if (!autoApproveWarningAccepted) {
				moreActions.push({
					label: localize('autoApprove.enable', '启用自动批准...'),
					data: {
						type: 'enable'
					}
				});
			...
			if (typeof data === 'boolean') {
				...
				switch (data.type) {
					...
					case 'newRule': {
						...
						function formatRuleLinks(newRules: ITerminalNewAutoApproveRule[]): string {
							return newRules.map(e => {
								...
								return `[\`${e.key}\`](${settingsUri.toString()} "${localize('ruleTooltip', '在设置中查看规则')}")`;
							}).join(', ');
						}
						...
						if (newRules.length === 1) {
							terminalData.autoApproveInfo = new MarkdownString(localize('newRule', '已添加自动批准规则 {0}', formatRuleLinks(newRules)), mdTrustSettings);
						} else if (newRules.length > 1) {
							terminalData.autoApproveInfo = new MarkdownString(localize('newRule.plural', '已添加自动批准规则 {0}', formatRuleLinks(newRules)), mdTrustSettings);
						}
						toolConfirmKind = ToolConfirmKind.UserAction;
						break;
					}
					...
					case 'sessionApproval': {
						...
						terminalData.autoApproveInfo = new MarkdownString(`${localize('sessionApproval', '本会话中所有命令将自动批准')} ([${localize('sessionApproval.disable', '禁用')}](${disableUri.toString()}))`, mdTrustSettings);
						toolConfirmKind = ToolConfirmKind.UserAction;
						break;
					}
				}
	...
	private _createButtons(moreActions: (IChatConfirmationButton<TerminalNewAutoApproveButtonData> | Separator)[] | undefined): IChatConfirmationButton<boolean | TerminalNewAutoApproveButtonData>[] {
		...
		return [
			{
				...getLabelAndTooltip(localize('tool.allow', "允许"), AcceptToolConfirmationActionId),
				data: true,
				moreActions,
			},
			{
				...getLabelAndTooltip(localize('tool.skip', "跳过"), SkipToolConfirmationActionId, localize('skip.detail', '不执行此命令继续')),
				data: { type: 'skip' },
				isSecondary: true,
			},
		];
	...
	private async _showAutoApproveWarning(): Promise<boolean> {
		const promptResult = await this.dialogService.prompt({
			type: Severity.Info,
			message: localize('autoApprove.title', '启用终端自动批准？'),
			buttons: [{
				label: localize('autoApprove.button.enable', '启用'),
				run: () => true
			}],
			cancelButton: true,
			custom: {
				icon: Codicon.shield,
				markdownDetails: [{
					markdown: new MarkdownString(localize('autoApprove.markdown', '这将启用可配置的命令子集在终端中自动运行。它提供*尽力保护*，并假设智能体没有恶意行为。')),
				}, {
					markdown: new MarkdownString(`[${localize('autoApprove.markdown2', '了解更多关于潜在风险及如何避免它们。')}](https://code.visualstudio.com/docs/copilot/security#_security-considerations)`)
				}],
			}
		});
