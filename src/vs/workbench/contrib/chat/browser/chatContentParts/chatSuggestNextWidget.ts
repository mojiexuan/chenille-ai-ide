...
export class ChatSuggestNextWidget extends Disposable {
	...
	public render(mode: IChatMode): void {
		...
		const modeName = mode.name.get() || mode.label.get() || localize('chat.currentMode', '当前模式');
		this.titleElement.textContent = localize('chat.proceedFrom', '从 {0} 继续', modeName);
	...
	private createPromptButton(handoff: IHandOff): HTMLElement {
		...
		if (showContinueOn && availableContributions.length > 0) {
			...
			dropdownContainer.setAttribute('aria-label', localize('chat.suggestNext.moreOptions', '{0} 的更多选项', handoff.label));
			...
			const showContextMenu = (e: MouseEvent | KeyboardEvent, anchor?: HTMLElement) => {
				...
				const actions = availableContributions.map(contrib => {
					...
					return new Action(
						contrib.type,
						localize('continueIn', "在 {0} 中继续", name),
