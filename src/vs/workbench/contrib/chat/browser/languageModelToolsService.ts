...
export class LanguageModelToolsService extends Disposable implements ILanguageModelToolsService {
	...
	constructor(
		...
		// Create the internal Chenille tool set
		this.vscodeToolSet = this._register(this.createToolSet(
			...
			VSCodeToolReference.vscode,
			{
				...
				description: localize('copilot.toolSet.vscode.description', '使用 Chenille 功能'),
			}
		));
		...
		this.executeToolSet = this._register(this.createToolSet(
			...
			SpecedToolAliases.execute,
			{
				...
				description: localize('copilot.toolSet.execute.description', '在您的计算机上执行代码和应用程序'),
			}
		));
		...
		this.readToolSet = this._register(this.createToolSet(
			...
			SpecedToolAliases.read,
			{
				...
				description: localize('copilot.toolSet.read.description', '读取工作区中的文件'),
			}
		));
	}
	...
	private async prepareToolInvocation(tool: IToolEntry, dto: IToolInvocation, token: CancellationToken): Promise<IPreparedToolInvocation | undefined> {
		...
		if (!isEligibleForAutoApproval && !prepared?.confirmationMessages?.title) {
			...
			prepared.confirmationMessages = {
				...prepared.confirmationMessages,
				title: localize('defaultToolConfirmation.title', '允许工具执行？'),
				message: localize('defaultToolConfirmation.message', '运行 \'{0}\' 工具？', fullReferenceName),
				disclaimer: new MarkdownString(localize('defaultToolConfirmation.disclaimer', '\'{0}\' 的自动批准受 {1} 限制。', getToolFullReferenceName(tool.data), createMarkdownCommandLink({ title: '`' + ChatConfiguration.EligibleForAutoApproval + '`', id: 'workbench.action.openSettings', arguments: [ChatConfiguration.EligibleForAutoApproval] }, false)), { isTrusted: true }),
		...
		if (!isEligibleForAutoApproval && prepared?.confirmationMessages?.title) {
			...
			prepared.confirmationMessages.disclaimer = new MarkdownString(localize('defaultToolConfirmation.disclaimer', '\'{0}\' 的自动批准受 {1} 限制。', getToolFullReferenceName(tool.data), createMarkdownCommandLink({ title: '`' + ChatConfiguration.EligibleForAutoApproval + '`', id: 'workbench.action.openSettings', arguments: [ChatConfiguration.EligibleForAutoApproval] }, false)), { isTrusted: true });
		}
	...
	private async _checkGlobalAutoApprove(): Promise<boolean> {
		...
		const promptResult = await this._dialogService.prompt({
			...
			message: localize('autoApprove2.title', '启用全局自动批准？'),
			buttons: [
				{
					label: localize('autoApprove2.button.enable', '启用'),
					run: () => true
				},
				{
					label: localize('autoApprove2.button.disable', '禁用'),
					run: () => false
				},
			],
			custom: {
