...
export class ModePickerActionItem extends ActionWidgetDropdownActionViewItem {
	constructor(
		...
		const builtInCategory = { label: localize('built-in', "内置"), order: 0 };
		const customCategory = { label: localize('custom', "自定义"), order: 1 };
		const policyDisabledCategory = { label: localize('managedByOrganization', "由您的组织管理"), order: 999, showHeader: true };
