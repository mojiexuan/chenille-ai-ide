		...
		getActions: () => {
			...
			if (models.length === 0) {
				...
				return [{
					...
					class: undefined,
					tooltip: localize('chat.modelPicker.auto', "自动"),
					label: localize('chat.modelPicker.auto', "自动"),
					run: () => { }
		...
		getActions: () => {
			...
			if (
				...
				additionalActions.push({
					id: 'manageModels',
					label: localize('chat.manageModels', "管理模型..."),
					enabled: true,
					tooltip: localize('chat.manageModels.tooltip', "管理语言模型"),
					class: undefined,
					run: () => {
			...
			if (isNewOrAnonymousUser || chatEntitlementService.entitlement === ChatEntitlement.Free) {
				additionalActions.push({
					id: 'moreModels',
					label: isNewOrAnonymousUser ? localize('chat.moreModels', "添加语言模型") : localize('chat.morePremiumModels', "添加高级模型"),
					enabled: true,
					tooltip: isNewOrAnonymousUser ? localize('chat.moreModels.tooltip', "添加语言模型") : localize('chat.morePremiumModels.tooltip', "添加高级模型"),
					class: undefined,
					run: () => {
...
export class ModelPickerActionItem extends ActionWidgetDropdownActionViewItem {
	constructor(
		...
		const actionWithLabel: IAction = {
			...action,
			label: currentModel?.metadata.name ?? localize('chat.modelPicker.auto', "自动"),
			tooltip: localize('chat.modelPicker.label', "选择模型"),
			run: () => { }
		};
	...
	protected override renderLabel(element: HTMLElement): IDisposable | null {
		...
		domChildren.push(dom.$('span.chat-model-label', undefined, this.currentModel?.metadata.name ?? localize('chat.modelPicker.auto', "自动")));
