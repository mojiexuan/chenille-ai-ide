...
export class RenameChatSessionAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize('renameSession', "重命名"),
			f1: false,
			...
			await chatSessionsService.setEditableSession(context.session.resource, {
				validationMessage: (value: string) => {
					if (!value || value.trim().length === 0) {
						return { content: localize('renameSession.emptyName', "名称不能为空"), severity: Severity.Error };
					}
					if (value.length > 100) {
						return { content: localize('renameSession.nameTooLong', "名称过长(最多 100 个字符)"), severity: Severity.Error };
					}
					return null;
				},
				placeholder: localize('renameSession.placeholder', "输入聊天会话的新名称"),
				startingValue: label,
				onFinish: async (value: string, success: boolean) => {
					if (success && value && value.trim() !== label) {
						try {
							...
							logService.error(
								localize('renameSession.error', "重命名聊天会话失败: {0}",
...
export class DeleteChatSessionAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize('deleteSession', "删除"),
			f1: false,
			category: CHAT_CATEGORY,
			icon: Codicon.x,
		});
	...
	async run(accessor: ServicesAccessor, context?: IMarshalledChatSessionContext): Promise<void> {
		...
		try {
			...
			const result = await dialogService.confirm({
				message: localize('deleteSession.confirm', "确定要删除此聊天会话吗?"),
				detail: localize('deleteSession.detail', "此操作无法撤销。"),
				primaryButton: localize('deleteSession.delete', "删除"),
				type: 'warning'
			});
...
export class OpenChatSessionInSidebarAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize('chat.openSessionInSidebar.label', "将聊天移动到侧边栏"),
			category: CHAT_CATEGORY,
			f1: false,
		});
	}
...
export class ToggleChatSessionsDescriptionDisplayAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize('chatSessions.toggleDescriptionDisplay.label', "显示详细描述"),
			category: CHAT_CATEGORY,
			f1: false,
...
export class ToggleAgentSessionsViewLocationAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize('chatSessions.toggleViewLocation.label', "合并会话视图"),
			category: CHAT_CATEGORY,
			f1: false,
...
MenuRegistry.appendMenuItem(MenuId.AgentSessionsContext, {
	command: {
		id: OpenChatSessionInSidebarAction.id,
		title: localize('openSessionInSidebar', "在侧边栏中打开")
	},
	group: 'navigation',
	order: 3,
...
MenuRegistry.appendMenuItem(MenuId.ViewTitle, {
	command: {
		...
		title: nls.localize2('interactiveSession.open', "新建聊天编辑器"),
		icon: Codicon.plus
	},
	group: 'navigation',
	order: 1,
