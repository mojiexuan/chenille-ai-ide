	...
	registerAction2(class OpenInChatViewAction extends Action2 {
		constructor() {
			super({
				...
				title: localize2('chat.openInChatView.label', "在聊天视图中打开"),
				f1: false,
	...
	registerAction2(class CloseQuickChatAction extends Action2 {
		constructor() {
			super({
				...
				title: localize2('chat.closeQuickChat.label', "关闭快速聊天"),
				f1: false,
				category: CHAT_CATEGORY,
				icon: Codicon.close,
				menu: {
...
class QuickChatGlobalAction extends Action2 {
	constructor() {
		super({
			...
			title: localize2('quickChat', '打开快速聊天'),
			...
			metadata: {
				description: localize('toggle.desc', '切换快速聊天'),
				args: [{
					name: 'args',
					schema: {
						anyOf: [
							{
								...
								properties: {
									query: {
										description: localize('toggle.query', "打开快速聊天时使用的查询"),
										type: 'string'
									},
									isPartialQuery: {
										description: localize('toggle.isPartialQuery', "查询是否为部分查询；将等待更多用户输入"),
										type: 'boolean'
									}
								},
							},
...
class AskQuickChatAction extends Action2 {
	constructor() {
		super({
			...
			category: CHAT_CATEGORY,
			title: localize2('interactiveSession.open', "打开快速聊天"),
			precondition: ChatContextKeys.enabled,
			f1: true
		});
	}
