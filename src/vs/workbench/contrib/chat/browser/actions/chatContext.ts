...
class ToolsContextPickerPick implements IChatContextPickerItem {
	...
	readonly label: string = localize('chatContext.tools', '工具...');
	...
	asPicker(widget: IChatWidget): IChatContextPicker {
		...
		return {
			placeholder: localize('chatContext.tools.placeholder', '选择工具'),
...
class OpenEditorContextValuePick implements IChatContextValueItem {
	...
	readonly label: string = localize('chatContext.editors', '打开的编辑器');
...
class RelatedFilesContextPickerPick implements IChatContextPickerItem {
	...
	readonly label: string = localize('chatContext.relatedFiles', '相关文件');
	...
	asPicker(widget: IChatWidget): IChatContextPicker {
		...
		return {
			placeholder: localize('relatedFiles', '将相关文件添加到工作集'),
			picks,
		};
	}
}
...
class ClipboardImageContextValuePick implements IChatContextValueItem {
	readonly type = 'valuePick';
	readonly label = localize('imageFromClipboard', '剪贴板中的图片');
	...
	async asAttachment(): Promise<IImageVariableEntry> {
		...
		return {
			...
			name: localize('pastedImage', '粘贴的图片'),
			fullName: localize('pastedImage', '粘贴的图片'),
			value: fileBuffer,
			kind: 'image',
		};
	}
...
export class TerminalContext implements IChatContextValueItem {
	...
	readonly label = localize('terminal', '终端');
...
class ScreenshotContextValuePick implements IChatContextValueItem {
	...
	readonly label = (isElectron
		? localize('chatContext.attachScreenshot.labelElectron.Window', '截取窗口')
		: localize('chatContext.attachScreenshot.labelWeb', '截图'));
