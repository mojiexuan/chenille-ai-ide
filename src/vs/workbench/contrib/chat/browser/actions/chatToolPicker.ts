...
function createToolSetTreeItem(toolset: ToolSet, checked: boolean, editorService: IEditorService): IToolSetTreeItem {
	...
	if (toolset.source.type === 'user') {
		...
		buttons.push({
			...
			tooltip: localize('editUserBucket', "编辑工具集"),
...
export async function showToolsPicker(
	...
	function computeItems(previousToolsEntries?: ReadonlyMap<ToolSet | IToolData, boolean>) {
			...
			if (source.type === 'mcp') {
				...
				if (collection?.source) {
					buttons.push({
						...
						tooltip: localize('configMcpCol', "配置 {0}", collection.label),
				...
				if (mcpServer.connectionState.get().state === McpConnectionState.Kind.Error) {
					buttons.push({
						...
						tooltip: localize('mcpShowOutput', "显示输出"),
				...
				return {
					...
					id: key,
					label: localize('defaultBucketLabel', "内置"),
					checked: undefined,
					children: [],
					buttons: [],
					collapsed: false,
				...
				return {
					...
					id: key,
					label: localize('userBucket', "用户定义的工具集"),
					checked: undefined,
					children: [],
					buttons: [],
					collapsed: true,
		...
		if (treeItems.length === 0) {
			treePicker.placeholder = localize('noTools', "向聊天添加工具");
		} else {
	...
	const updateToolLimitMessage = () => {
		if (toolLimit) {
			...
			if (count > toolLimit) {
				...
				treePicker.validationMessage = localize('toolLimitExceeded', "已启用 {0} 个工具。超过 {1} 个工具可能会导致工具调用性能下降。", count, createMarkdownCommandLink({ title: String(toolLimit), id: '_chat.toolPicker.closeAndOpenVirtualThreshold' }));
			} else {
	...
	const addMcpServerButton = {
		...
		tooltip: localize('addMcpServer', '添加 MCP 服务器...')
	};
	const installExtension = {
		iconClass: ThemeIcon.asClassName(Codicon.extensions),
		tooltip: localize('addExtensionButton', '安装扩展...')
	};
	const configureToolSets = {
		iconClass: ThemeIcon.asClassName(Codicon.gear),
		tooltip: localize('configToolSets', '配置工具集...')
	};
	treePicker.title = localize('configureTools', "配置工具");
