...
export class AgentSessionsFilter extends Disposable implements Required<IAgentSessionsFilter> {
	...
	private registerStateActions(disposables: DisposableStore): void {
		const states: { id: ChatSessionStatus; label: string }[] = [
			{ id: ChatSessionStatus.Completed, label: localize('chatSessionStatus.completed', "已完成") },
			{ id: ChatSessionStatus.InProgress, label: localize('chatSessionStatus.inProgress', "进行中") },
			{ id: ChatSessionStatus.Failed, label: localize('chatSessionStatus.failed', "失败") },
			{ id: ChatSessionStatus.NeedsInput, label: localize('chatSessionStatus.needsInput', "需要输入") },
		];
		...
		disposables.add(registerAction2(class extends Action2 {
			constructor() {
				super({
					...
					title: localize('agentSessions.filter.archived', '已归档'),
					menu: {
						id: that.options.filterMenuId,
						group: '3_props',
						order: 1000,
		...
		disposables.add(registerAction2(class extends Action2 {
			constructor() {
				super({
					...
					title: localize('agentSessions.filter.read', '已读'),
					menu: {
						id: that.options.filterMenuId,
						group: '3_props',
						order: 0,
		...
		disposables.add(registerAction2(class extends Action2 {
			constructor() {
				super({
					...
					title: localize('agentSessions.filter.reset', "重置"),
					menu: {
						id: that.options.filterMenuId,
						group: '4_reset',
						order: 0,
