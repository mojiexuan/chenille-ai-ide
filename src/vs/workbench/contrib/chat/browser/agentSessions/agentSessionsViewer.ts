	...
	private renderDescription(session: ITreeNode<IAgentSession, FuzzyScore>, template: IAgentSessionItemTemplate): void {
		...
		// Fallback to state label
			...
			if (this.chatSessionsService.isChatSessionInProgressStatus(session.element.status)) {
				template.description.textContent = localize('chat.session.status.inProgress', "正在处理...");
			} else if (
				...
				template.description.textContent = session.element.status === ChatSessionStatus.Failed ?
					localize('chat.session.status.failedAfter', "{0}后失败。", duration ?? '1s') :
					localize('chat.session.status.completedAfter', "{0}内完成。", duration ?? '1s');
			} else {
				template.description.textContent = session.element.status === ChatSessionStatus.Failed ?
					localize('chat.session.status.failed', "失败") :
					localize('chat.session.status.completed', "已完成");
			}
		}
	}
...
export class AgentSessionsAccessibilityProvider implements IListAccessibilityProvider<IAgentSession> {
	...
	getWidgetAriaLabel(): string {
		return localize('agentSessions', "智能体会话");
	}
