...
export class AgentSessionsView extends ViewPane {
	...
	private registerListeners(): void {
		...
		this._register(sessionsModel.onWillResolve(() => {
			...
			this.progressService.withProgress(
				{
					location: this.id,
					title: localize('agentSessions.refreshing', '正在刷新智能体会话...'),
					delay: 500
				},
				() => didResolve.p
			);
	...
	private createNewSessionButton(container: HTMLElement): void {
		...
		const newSessionButton = this._register(new ButtonWithDropdown(this.newSessionContainer, {
			title: localize('agentSessions.newSession', "新建会话"),
			ariaLabel: localize('agentSessions.newSessionAriaLabel', "新建会话"),
		...
		newSessionButton.label = localize('agentSessions.newSession', "新建会话");
	...
	private getNewSessionActions(): IAction[] {
		...
		actions.push(toAction({
			...
			label: localize('newChatSessionDefault', "新建本地会话"),
		...
		actions.push(toAction({
			...
			label: localize('newBackgroundSession', "新建后台会话"),
		...
		actions.push(toAction({
			...
			label: localize('newCloudSession', "新建云端会话"),
		...
		for (const provider of this.chatSessionsService.getAllChatSessionContributions()) {
			...
			// ...over our generic one
				...
				actions.push(toAction({
					...
					label: localize('newChatSessionFromProvider', "新建 {0}", provider.displayName),
