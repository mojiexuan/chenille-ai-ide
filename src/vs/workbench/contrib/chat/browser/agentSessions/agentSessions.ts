...
export function getAgentSessionProviderName(provider: AgentSessionProviders): string {
	switch (provider) {
		case AgentSessionProviders.Local:
			return localize('chat.session.providerLabel.local', "本地");
		case AgentSessionProviders.Background:
			return localize('chat.session.providerLabel.background', "后台");
		case AgentSessionProviders.Cloud:
			return localize('chat.session.providerLabel.cloud', "云端");
	}
}
...
export const agentSessionReadIndicatorForeground = registerColor(
	...
	localize('agentSessionReadIndicatorForeground', "智能体会话中已读指示器的前景色。")
);
