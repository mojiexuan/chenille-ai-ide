...
abstract class NavigateAction extends ChatEditingEditorAction {
	...
	constructor(readonly next: boolean) {
		super({
			...
			title: next
				? localize2('next', '转到下一个聊天编辑')
				: localize2('prev', '转到上一个聊天编辑'),
...
abstract class KeepOrUndoAction extends ChatEditingEditorAction {
	...
	constructor(id: string, private _keep: boolean) {
		super({
			id,
			title: _keep
				? localize2('accept', '保留聊天编辑')
				: localize2('discard', '撤销聊天编辑'),
			shortTitle: _keep
				? localize2('accept2', '保留')
				: localize2('discard2', '撤销'),
			tooltip: _keep
				? localize2('accept3', '保留此文件中的聊天编辑')
				: localize2('discard3', '撤销此文件中的聊天编辑'),
		...
		super(
			{
				...
				title: _accept ? localize2('acceptHunk', '保留此更改') : localize2('undo', '撤销此更改'),
				shortTitle: _accept ? localize2('acceptHunkShort', '保留') : localize2('undoShort', '撤销'),
...
class ToggleDiffAction extends ChatEditingEditorAction {
	constructor() {
		super({
			...
			title: localize2('diff', '切换聊天编辑的差异编辑器'),
			category: CHAT_CATEGORY,
			toggled: {
...
class ToggleAccessibleDiffViewAction extends ChatEditingEditorAction {
	constructor() {
		super({
			...
			title: localize2('accessibleDiff', '显示聊天编辑的无障碍差异视图'),
			f1: true,
			precondition: ContextKeyExpr.and(ctxHasEditorModification, ctxIsCurrentlyBeingModified.negate()),
			keybinding: {
				when: EditorContextKeys.focus,
...
export class AcceptAllEditsAction extends ChatEditingEditorAction {
	...
	constructor() {
		super({
			...
			title: localize2('acceptAllEdits', '保留所有聊天编辑'),
			tooltip: localize2('acceptAllEditsTooltip', '保留此会话中的所有聊天编辑'),
			precondition: ContextKeyExpr.and(ctxHasEditorModification, ctxIsCurrentlyBeingModified.negate()),
...
abstract class MultiDiffAcceptDiscardAction extends Action2 {
	...
	constructor(readonly accept: boolean) {
		super({
			...
			title: accept ? localize('accept4', '保留所有编辑') : localize('discard4', '撤销所有编辑'),
