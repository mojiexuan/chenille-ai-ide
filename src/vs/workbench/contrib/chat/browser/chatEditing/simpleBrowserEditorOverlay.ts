...
class SimpleBrowserOverlayWidget {
	...
	constructor(
		...
		const startSelectionMessage = localize('elementSelectionMessage', '将元素添加到聊天');
		...
		actions.push(
			toAction({
				id: 'singleSelection',
				label: localize('selectElementDropdown', '选择元素'),
				enabled: true,
				run: async () => { await startElementSelection(); }
			}),
			toAction({
				id: 'continuousSelection',
				label: localize('continuousSelectionDropdown', '连续选择'),
				enabled: true,
				run: async () => {
					this._editor.focus();
					cts = new CancellationTokenSource();
					// start selection
					message.textContent = localize('elementSelectionInProgress', '正在选择元素...');
					this.hideElement(startButton.element);
					this.showElement(cancelButton.element);
					cancelButton.label = localize('finishSelectionLabel', '完成');
					...
					// stop selection
					message.textContent = localize('elementSelectionComplete', '元素已添加到聊天');
					finishedSelecting();
				}
			}));
		...
		const startButton = this._showStore.add(new ButtonWithDropdown(mainContent, {
			...
			title: localize('selectAnElement', '点击选择元素。'),
			supportIcons: true,
			...defaultButtonStyles
		}));

		startButton.primaryButton.label = localize('startSelection', '开始');
		startButton.element.classList.add('element-selection-start');

		const cancelButton = this._showStore.add(new Button(mainContent, { ...defaultButtonStyles, supportIcons: true, title: localize('cancelSelection', '点击取消选择。') }));
		cancelButton.element.className = 'element-selection-cancel hidden';
		const cancelButtonLabel = localize('cancelSelectionLabel', '取消');
		cancelButton.label = cancelButtonLabel;

		const configure = this._showStore.add(new Button(mainContent, { supportIcons: true, title: localize('chat.configureElements', "配置发送的附件") }));
		configure.icon = Codicon.gear;

		const collapseOverlay = this._showStore.add(new Button(mainContent, { supportIcons: true, title: localize('chat.hideOverlay', "折叠覆盖层") }));
		collapseOverlay.icon = Codicon.chevronRight;

		const nextSelection = this._showStore.add(new Button(mainContent, { supportIcons: true, title: localize('chat.nextSelection', "再次选择") }));
		...
		const expandOverlay = this._showStore.add(new Button(expandContainer, { supportIcons: true, title: localize('chat.expandOverlay', "展开覆盖层") }));
		...
		this._showStore.add(addDisposableListener(cancelButton.element, 'click', () => {
			cts.cancel();
			message.textContent = localize('elementCancelMessage', '选择已取消');
			finishedSelecting();
		}));
...
class SimpleBrowserOverlayController {
	...
	constructor(
		...
		const show = async () => {
			...
			connectingWebviewElement.textContent = localize('connectingWebviewElement', '正在连接到 webview...');
			...
			if (activeBrowserType) {
				try {
					...
					connectingWebviewElement.textContent = localize('reopenErrorWebviewElement', '请重新打开预览。');
					return;
				}
			}
