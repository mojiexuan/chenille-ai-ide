...
export class ChatEditingAcceptAllAction extends EditingSessionAction {
	...
	constructor() {
		super({
			...
			title: localize('accept', '保留'),
			icon: Codicon.check,
			tooltip: localize('acceptAllEdits', '保留所有编辑'),
...
export class ChatEditingDiscardAllAction extends EditingSessionAction {
	...
	constructor() {
		super({
			...
			title: localize('discard', '撤消'),
			icon: Codicon.discard,
			tooltip: localize('discardAllEdits', '撤消所有编辑'),
			precondition: hasUndecidedChatEditingResourceContextKey,
			menu: [
				{
					id: MenuId.ChatEditingWidgetToolbar,
...
export async function discardAllEditsWithConfirmation(accessor: ServicesAccessor, currentEditingSession: IChatEditingSession): Promise<boolean> {
	...
	if (entries.length > 0) {
		const confirmation = await dialogService.confirm({
			title: localize('chat.editing.discardAll.confirmation.title', "撤消所有编辑？"),
			message: entries.length === 1
				? localize('chat.editing.discardAll.confirmation.oneFile', "这将撤消在 {0} 中所做的更改。是否继续？", basename(entries[0].modifiedURI))
				: localize('chat.editing.discardAll.confirmation.manyFiles', "这将撤消在 {0} 个文件中所做的更改。是否继续？", entries.length),
			primaryButton: localize('chat.editing.discardAll.confirmation.primaryButton', "是"),
			type: 'info'
		});
...
export class ChatEditingShowChangesAction extends EditingSessionAction {
	...
	static readonly LABEL = localize('chatEditing.viewChanges', '查看所有编辑');
	...
	override async run(accessor: ServicesAccessor, sessionResource?: URI): Promise<void> {
		...
		if (resources.length > 0) {
			await commandService.executeCommand('_workbench.openMultiDiffEditor', {
				...
				title: localize('chatEditing.allChanges.title', '所有会话更改'),
				resources,
			});
		}
	}
	...
	if (requestId) {
		...
		if (editsToUndo === 1) {
			if (entriesModifiedInRequestsToRemove.length === 1) {
				message = localize('chat.removeLast.confirmation.message2', "这将移除您的上一个请求并撤消对 {0} 所做的编辑。是否继续？", basename(entriesModifiedInRequestsToRemove[0].modifiedURI));
			} else {
				message = localize('chat.removeLast.confirmation.multipleEdits.message', "这将移除您的上一个请求并撤消对工作集中 {0} 个文件所做的编辑。是否继续？", entriesModifiedInRequestsToRemove.length);
			}
		} else {
			if (entriesModifiedInRequestsToRemove.length === 1) {
				message = localize('chat.remove.confirmation.message2', "这将移除所有后续请求并撤消对 {0} 所做的编辑。是否继续？", basename(entriesModifiedInRequestsToRemove[0].modifiedURI));
			} else {
				message = localize('chat.remove.confirmation.multipleEdits.message', "这将移除所有后续请求并撤消对工作集中 {0} 个文件所做的编辑。是否继续？", entriesModifiedInRequestsToRemove.length);
			}
		}
		...
		const confirmation = shouldPrompt
			? await dialogService.confirm({
				title: editsToUndo === 1
					? localize('chat.removeLast.confirmation.title', "是否要撤消上一次编辑？")
					: localize('chat.remove.confirmation.title', "是否要撤消 {0} 次编辑？", editsToUndo),
				message: message,
...
registerAction2(class OpenWorkingSetHistoryAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize('chat.openFileUpdatedBySnapshot.label', "打开文件"),
			menu: [{
...
registerAction2(class OpenWorkingSetHistoryAction extends Action2 {
	...
	constructor() {
		super({
			id: OpenWorkingSetHistoryAction.id,
			title: localize('chat.openSnapshot.label', "打开文件快照"),
			menu: [{
				id: MenuId.ChatEditingCodeBlockContext,
				group: 'navigation',
				order: 1,
...
export class ViewPreviousEditsAction extends EditingSessionAction {
	...
	static readonly Label = localize('chatEditing.viewPreviousEdits', '查看之前的编辑');
