...
export class ChatDragAndDrop extends Themable {
	...
	private getDropTypeName(type: ChatDragAndDropType): string {
		switch (type) {
			case ChatDragAndDropType.FILE_INTERNAL: return localize('file', '文件');
			case ChatDragAndDropType.FILE_EXTERNAL: return localize('file', '文件');
			case ChatDragAndDropType.FOLDER: return localize('folder', '文件夹');
			case ChatDragAndDropType.IMAGE: return localize('image', '图片');
			case ChatDragAndDropType.SYMBOL: return localize('symbol', '符号');
			case ChatDragAndDropType.MARKER: return localize('problem', '问题');
			case ChatDragAndDropType.HTML: return localize('url', 'URL');
			case ChatDragAndDropType.NOTEBOOK_CELL_OUTPUT: return localize('notebookOutput', '输出');
			case ChatDragAndDropType.SCM_HISTORY_ITEM: return localize('scmHistoryItem', '更改');
		}
	}
	...
	private async resolveHTMLAttachContext(e: DragEvent): Promise<IChatRequestVariableEntry[]> {
		...
		const createDisplayName = (): string => {
			const baseName = localize('dragAndDroppedImageName', '来自 URL 的图片');
	...
	private getOverlayText(type: ChatDragAndDropType): string {
		...
		return localize('attacAsContext', '将{0}附加为上下文', typeName);
	}
