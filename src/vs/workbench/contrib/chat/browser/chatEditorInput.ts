...
const ChatEditorIcon = registerIcon('chat-editor-label-icon', Codicon.chatSparkle, nls.localize('chatEditorLabelIcon', '聊天编辑器标签图标。'));
...
export class ChatEditorInput extends EditorInput implements IEditorCloseHandler {
	...
	async confirm(editors: ReadonlyArray<IEditorIdentifier>): Promise<ConfirmResult> {
		...
		const titleOverride = nls.localize('chatEditorConfirmTitle', "关闭聊天编辑器");
		const messageOverride = nls.localize('chat.startEditing.confirmation.pending.message.default', "关闭聊天编辑器将结束当前的编辑会话。");
	...
	override getName(): string {
		...
		const defaultName = this.options.title?.fallback ?? nls.localize('chatEditorName', "聊天");
...
export async function showClearEditingSessionConfirmation(model: IChatModel, dialogService: IDialogService, options?: IClearEditingSessionConfirmationOptions): Promise<boolean> {
	...
	const defaultPhrase = nls.localize('chat.startEditing.confirmation.pending.message.default1', "开始新聊天将结束当前的编辑会话。");
	const defaultTitle = nls.localize('chat.startEditing.confirmation.title', "开始新聊天？");
	...
	const { result } = await dialogService.prompt({
		title,
		message: phrase + ' ' + nls.localize('chat.startEditing.confirmation.pending.message.2', "是否保留对 {0} 个文件的待处理编辑？", undecidedEdits.length),
		type: 'info',
		cancelButton: true,
		buttons: [
			{
				label: nls.localize('chat.startEditing.confirmation.acceptEdits', "保留并继续"),
				run: async () => {
			...
			{
				label: nls.localize('chat.startEditing.confirmation.discardEdits', "撤消并继续"),
				run: async () => {
