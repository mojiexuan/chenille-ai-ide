...
export class ChatStatusWidget extends Disposable implements IChatInputPartWidget {
	...
	private createWidgetContent(enabledSku: 'free' | 'anonymous'): void {
		...
		if (enabledSku === 'anonymous') {
			const message = localize('chat.anonymousRateLimited.message', "您已达到聊天消息的限制。免费试用 Copilot Pro。");
			const buttonLabel = localize('chat.anonymousRateLimited.signIn', "登录");
			...
			const message = localize('chat.freeQuotaExceeded.message', "您已达到聊天消息的限制。");
			const buttonLabel = localize('chat.freeQuotaExceeded.upgrade', "升级");
			this.messageElement.textContent = message;
			this.actionButton.label = buttonLabel;
