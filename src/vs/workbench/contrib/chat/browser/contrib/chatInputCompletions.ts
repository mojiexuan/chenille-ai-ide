	...
	constructor(
		...
		this._register(this.languageFeaturesService.completionProvider.register({ scheme: Schemas.vscodeChatInput, hasAccessToAllModels: true }, {
			...
			provideCompletionItems: async (model: ITextModel, position: Position, _context: CompletionContext, token: CancellationToken) => {
				...
				const label = localize('installLabel', "安装聊天扩展...");
				const item: CompletionItem = {
					label,
					insertText: '',
					range,
		...
		try {
			...
			try {
				...
				if (!cts.token.isCancellationRequested) {
					notificationService.error(localize('mcp.prompt.error', "解析提示时出错：{0}", String(e)));
				}
				replaceTextWith('');
				return;
			}
				...
				if (isText) {
					if (validURI) {
						...
						toAttach.push({
							...
							kind: 'generic',
							value: contents,
							name: localize('mcp.prompt.resource', '提示资源'),
						});
					}
					...
					chatWidget.attachmentModel.addContext({
						id: generateUuid(),
						name: localize('mcp.prompt.image', '提示图片'),
						fullName: localize('mcp.prompt.image', '提示图片'),
						value: resized,
						kind: 'image',
...
class BuiltinDynamicCompletions extends Disposable {
	...
	private async addFileAndFolderEntries(widget: IChatWidget, result: CompletionList, info: { insert: Range; replace: Range; varWord: IWordAtPosition | null }, token: CancellationToken) {
		...
		for (const [i, item] of this.historyService.getHistory().entries()) {
			...
			const newLen = result.suggestions.push(makeCompletionItem(item.resource, FileKind.FILE, i === 0 ? localize('activeFile', '活动文件') : undefined, i === 0));
			if (newLen - len >= 5) {
				break;
			}
		}
