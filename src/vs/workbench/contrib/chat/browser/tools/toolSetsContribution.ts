...
const toolSetsSchema: IJSONSchema = {
	...
	additionalProperties: {
		...
		properties: {
			tools: {
				description: localize('schema.tools', "要包含在此工具集中的工具或工具集列表。不能为空，且必须按照提示中引用工具的方式引用工具。"),
				type: 'array',
				minItems: 1,
				items: {
					type: 'string',
			...
			icon: {
				description: localize('schema.icon', '在 UI 中用于此工具集的图标。使用 "\\$(name)" 语法，如 "\\$(zap)"'),
				type: 'string',
				enum: Array.from(getAllCodicons(), icon => icon.id),
				markdownEnumDescriptions: Array.from(getAllCodicons(), icon => `$(${icon.id})`),
			},
			description: {
				description: localize('schema.description', "此工具集的简短描述。"),
				type: 'string'
			},
		},
	}
	...
	constructor(
		...
		this._store.add(autorun(r => {
			...
			for (const item of data) {
				...
				toolEnumDescriptions.push(localize('tool.description', "{1} ({0})\n\n{2}", item.sourceLabel, item.name, item.description)); // 保留原格式，参数为动态值
			}
...
export class ConfigureToolSets extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('chat.configureToolSets', '配置工具集...'),
			shortTitle: localize('chat.configureToolSets.short', "工具集"),
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		picks.push({
			label: localize('chat.configureToolSets.add', '创建新的工具集文件...'),
			alwaysShow: true,
		...
		const pick = await quickInputService.pick(picks, {
			canPickMany: false,
			placeHolder: localize('chat.configureToolSets.placeholder', '选择要配置的工具集'),
		});
		...
		if (!pick.toolset) {
			...
			const name = await quickInputService.input({
				placeHolder: localize('input.placeholder', "输入工具集文件名"),
				validateInput: async (input) => {
					if (!input) {
						return localize('bad_name1', "无效的文件名");
					}
					if (!isValidBasename(input)) {
						return localize('bad_name2', "'{0}' 不是有效的文件名", input);
					}
					return undefined;
				}
			});
