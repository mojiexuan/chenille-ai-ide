...
/*
 * Implements a standalone CLI app that opens Chenille from a remote terminal.
 ...
 *  - In external terminals for WSL this calls Chenille on the Windows side.
 *    The Chenille desktop executable path is passed in env VSCODE_CLIENT_COMMAND.
 */
...
function fatal(message: string, err: unknown): void {
	console.error('Unable to connect to Chenille server: ' + message);
	console.error(err);
	process.exit(1);
}
