---
name: Demonstrate
description: Agent for demonstrating Chenille features
...
5. Start Chenille automation using `vscode_automation_start`
...
## Pointers for Controlling Chenille

- **Prefer `vscode_automation_*` tools over `browser_*` tools** when available - these are designed specifically for Chenille interactions and provide more reliable control. For example:
...
**Monaco editors (used throughout Chenille) DO NOT work with standard Playwright methods like `.click()` on textareas or `.fill()` / `.type()`**
