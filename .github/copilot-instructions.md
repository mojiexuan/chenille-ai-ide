...
- `extensions/`: Built-in extensions that ship with Chenille
...
- `src/vs/workbench/` - Main application workbench for web and desktop
  ...
  - `workbench/api/` - Extension host and Chenille API implementation
...
The `extensions/` directory contains first-party extensions that ship with Chenille:
...
Each extension follows the standard Chenille extension structure with `package.json`, TypeScript sources, and contribution points to extend the workbench through the Extension API.
...
MANDATORY: Always check the `Chenille - Build` watch task output via #runTasks/getTaskOutput for compilation errors before running ANY script or declaring work complete, then fix all compilation errors before moving forward.
...
- Monitor the `Chenille - Build` task outputs for real-time compilation errors as you make changes
- This task runs `Core - Build` and `Ext - Build` to incrementally compile Chenille TypeScript sources and built-in extensions
