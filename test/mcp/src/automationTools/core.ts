...
export function applyCoreTools(server: McpServer, appService: ApplicationService): RegisteredTool[] {
	...
	// 	'Restart Chenille with optional workspace or folder and extra arguments',
	// 	{
	...
	// 			content: [{
	// 				type: 'text' as const,
	// 				text: `Chenille restarted successfully${workspaceOrFolder ? ` with workspace: ${workspaceOrFolder}` : ''}`
	// 			}]
	// 		};
	// 	}
	// );
	...
	tools.push(server.tool(
		...
		async () => {
			...
			return {
				content: [{
					type: 'text' as const,
					text: 'Chenille stopped successfully'
				}]
			};
		}
	));
	...
	// 	'Get the quality/build type of Chenille (Dev, Insiders, Stable, etc.)',
	// 	async () => {
	// 		const info = {
	// 			quality: app.quality,
	// 			remote: app.remote,
	...
	// 	'Send a keybinding to Chenille (e.g., ctrl+shift+p, cmd+s)',
	// 	{
